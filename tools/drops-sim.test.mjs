...
drops.spawnGear(92.5, SURF - 1, { id: 'cape_x', kind: 'cape', name: 'Y', tier: 'rare', airJumps: 1, fabric: 'ember' }, { vx: 0, vy: 0 });
drops.spawnGear(94.5, SURF - 1, { id: 'guardian_frost_test', kind: 'weapon', weaponType: 'bow', name: 'Frost', tier: 'epic', attackDamage: 12, mergePerk: 'frost', fabric: 'silk' }, { vx: 0, vy: 0 });
drops.spawnGear(96.5, SURF - 1, { id: 'cape_fab_bad', kind: 'cape', name: 'Z', tier: 'rare', airJumps: 1, fabric: 'kevlar' }, { vx: 0, vy: 0 });
const snap = drops.snapshot();
assert.equal(snap.list.length, 4, 'snapshot captures resources and every gear drop');
drops.reset();
drops.restore(snap);
assert.equal(drops.metrics().active, 4, 'restore brings every drop back');
const restoredGear = drops._debug.list.find(d => d.kind === 'gear' && d.item && d.item.id === 'cape_x');
assert.equal(restoredGear.item.id, 'cape_x', 'gear payload survives the roundtrip');
assert.equal(restoredGear.item.fabric, 'ember', 'a cape drop keeps its whitelisted fabric through drop persistence');
...
assert.equal(restoredFrost.item.fabric, undefined, 'fabric never rides a non-cape drop');
const restoredBadFab=drops._debug.list.find(d=>d.kind==='gear' && d.item && d.item.id==='cape_fab_bad');
assert.equal(restoredBadFab.item.fabric, undefined, 'an unlisted fabric never survives drop persistence');
