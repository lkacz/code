...
const resonator = dbg.entities.find(e=>e.resonator);
assert.ok(resonator, 'debug exposes an active air resonator');
assert.equal(skyGuardian.damageAt(Math.floor(resonator.x), Math.floor(resonator.y), 20, {kind:'electric',element:'electric',source:'hero'}), true, 'electric damage can hit an air resonator');
assert.ok(combatEvents.some(e=>e && e.target==='guardian' && e.element==='electric' && e.bonusDamagePct===45), 'electric resonator weakness emits a shock bonus combat event');
