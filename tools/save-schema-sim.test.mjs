...
assert.match(src, /function canMineBedrockWithCurrentTool\(t\)\{ return t===T\.BEDROCK && player\.tool==='bedrock' && hasBedrockPick\(\); \}/, 'bedrock mining is gated to the active mother pickaxe');
assert.match(src, /function canMineTileWithCurrentTool\(t\)\{ return !isUnmineableTile\(t\) \|\| canMineBedrockWithCurrentTool\(t\); \}/, 'mining centralizes unmineable tile checks with the bedrock-pick exception');
assert.match(src, /if\(!canMineTileWithCurrentTool\(t\)\)\{ if\(!quiet\) msg\(unmineableReason\(t\)\); return false; \}/, 'cursor mining rejects unmineable tiles before timers start');
assert.match(src, /if\(t===T\.AIR \|\| !canMineTileWithCurrentTool\(t\)\)/, 'instant break cannot bypass unmineable tiles');
assert.match(src, /if\(info\.unmineable && !canMineBedrockWithCurrentTool\(tId\)\) return false;/, 'breakMinedTile refuses unmineable terrain except the bedrock pickaxe path');
