...
const lowRollRoles = assignRoles(8, profile, ()=>0.01);
const highRollRoles = assignRoles(8, profile, ()=>0.99);
assert.notDeepEqual(lowRollRoles, highRollRoles, 'same-sized regular squads can roll different specialist compositions');
assert.ok(!lowRollRoles.includes('commander') && !highRollRoles.includes('commander'), 'commanders only enter through the host promotion roll');
