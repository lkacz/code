...
assert.match(mainSource, /function drawMotherIceArt\(g,px,py,h\)/, 'mother ice has bespoke faceted relic art');
assert.match(mainSource, /function drawMotherLavaArt\(g,px,py,h\)/, 'mother lava has bespoke crust and magma-heart art');
assert.match(mainSource, /function drawGraphiteArt\(g,px,py,h\)/, 'graphite has layered crystal-plane art');
assert.match(mainSource, /function drawGrapheneArt\(g,px,py,h\)/, 'graphene has a distinctive hexagonal lattice');
assert.match(mainSource, /function drawSmrCellArt\(g,px,py,h\)/, 'SMR cells have a complete reactor-cassette silhouette');
assert.match(mainSource, /function drawMeteorDustTileArt\(g,px,py,wx,wy,h,conn\)/, 'meteor dust has a coordinate-aware ethereal renderer instead of a filled square');
assert.match(mainSource, /floating deposits merge into one branching nebula instead of tiled piles/, 'meteor dust art is designed as connected suspended sky matter');
assert.match(mainSource, /if\(conn\.left\)[\s\S]*if\(conn\.right\)[\s\S]*if\(conn\.up\)[\s\S]*if\(conn\.down\)/, 'meteor dust draws tendrils toward every adjacent dust cell');
assert.match(mainSource, /function drawAntimatterCrystalArt\(g,px,py,h\)/, 'antimatter has a dedicated multi-shard crystal renderer');
...
assert.match(mainSource, /const dustConn=\{[\s\S]*left:chunkTileAt[\s\S]*right:chunkTileAt[\s\S]*up:chunkTileAt[\s\S]*down:chunkTileAt[\s\S]*drawMeteorDustTileArt\(cctx,lx\*TILE,y\*TILE,wx,y,hash32\(wx,y\),dustConn\);/, 'placed meteor dust connects its transparent nebula art across neighboring cells');
assert.match(mainSource, /t===T\.ANTIMATTER_CRYSTAL[\s\S]*drawMaterialIdentityArt\(cctx,t,lx\*TILE,y\*TILE,h\);/, 'placed antimatter routes through the detailed crystal renderer');
...
assert.match(mainSource, /prioritizeVisibleRenderSections\(visibleRenderSections,centerChunk,centerSection\);[\s\S]*for\(const ref of visibleRenderSections\) drawChunkToCache\(ref\.cx,ref\.section,centerChunk\);/, 'visible chunk caches spend their rebuild budget from the hero section outward');
assert.match(mainSource, /entry\.version===currentVersion && !entry\.edgeStale && !hasPixelDirty/, 'same-version temporal restores cannot discard a pending pixel invalidation');
assert.match(mainSource, /canRebuildChunkCache\(entry,currentVersion,hasPixelDirty\)/, 'same-version dirty canvases remain eligible for a rebuild');
