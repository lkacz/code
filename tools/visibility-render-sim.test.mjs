...
assert.match(mainSource, /function renderDetailFor\(z,viewX,viewY\)/, 'renderer derives a zoom-out detail mode from visible tile count');
assert.match(mainSource, /fogStep:tier>=2 \? 3 : \(tier>=1 \? 2 : 1\)/, 'zoomed-out fog uses a coarser overlay grid');
assert.match(mainSource, /currentRenderDetail\.grass && VISUAL\.animations && GRASS && GRASS\.drawOverlays/, 'subpixel grass overlays are skipped in wide zoom detail modes');
assert.match(mainSource, /lodStep:currentRenderDetail\.fogStep/, 'fog overlay receives the active zoom detail step');
const fogSource = readFileSync(new URL('../src/engine/fog.js', import.meta.url), 'utf8');
assert.match(fogSource, /const lodStep=Math\.max\(1, Math\.min\(4/, 'fog overlay clamps its zoom-out LOD step');
assert.match(fogSource, /const blockStyle=\(x,y,step\)=>/, 'fog overlay can aggregate coarse cells for zoomed-out rendering');
assert.match(fogSource, /if\(lodStep>1\)\{[\s\S]*drawRect\(\{x0:runStart,x1:x,y0:y,y1,style:runStyle\}\);[\s\S]*return;/, 'fog overlay exits through a coarse run path when zoomed far out');
