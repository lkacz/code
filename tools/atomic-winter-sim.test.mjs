...
const startMetrics = atomicWinter.metrics();
assert.equal(startMetrics.tLeft, 6000, 'atomic winter timer starts with one full winter season');
assert.equal(startMetrics.duration, 6000, 'metrics expose the full timer duration for the HUD');
assert.equal(startMetrics.progress, 1, 'timer progress starts full');
...
assert.ok(messages.some(m=>m.includes('Zaczęła się zima nuklearna')), 'world event announces the atomic winter trigger');
assert.ok(messages.some(m=>m.includes('Trujący deszcz')), 'world event explains the most important fallout danger');
...
assert.ok(atomicWinter.metrics().progress < 1, 'timer progress falls as simulation time passes');
...
assert.equal(atomicWinter.metrics().tLeft, snap.tLeft, 'restore preserves the exact remaining timer');
...
assert.equal(atomicWinter.metrics().progress, 0, 'expired fallout exposes an empty timer progress');
...
assert.match(mainSrc, /function formatAtomicWinterCountdown\(value\)/, 'main has a stable hours-minutes-seconds countdown formatter');
assert.match(mainSrc, /function updateAtomicWinterTimerHud\(\)[\s\S]*ATOMIC_WINTER\.metrics\(\)[\s\S]*panel\.hidden=!active/, 'HUD visibility follows live atomic-winter metrics');
assert.match(mainSrc, /const seconds=Math\.ceil\(tLeft\)/, 'countdown rounds up so it never shows zero before expiry');
assert.match(mainSrc, /updateStatusHud\(ts\)[\s\S]*updateAtomicWinterTimerHud\(\)/, 'the regular throttled HUD refresh updates the fallout timer');
assert.match(indexSrc, /id="atomicWinterTimerPanel"[^>]*role="timer"[^>]*hidden/, 'HUD contains an initially hidden accessible timer panel');
