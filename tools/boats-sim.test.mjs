...
assert.match(mainSrc, /const ridingFloatingBoat = !!\(heroBoatNow && heroBoatNow\.inWater && !heroBoatNow\.grounded\);[\s\S]*const swimming = inWater && !player\.onGround && !ridingFloatingBoat && !godMode;/, 'standing on a floating raft suppresses swim-chill damage');
