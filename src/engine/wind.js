...
(function(){
  ...
  const CFG = {
    ...
    HERO_AIR_ACCEL: 1.15,        // horizontal acceleration multiplier while airborne
  ...
  function isWindBlocker(t){
    ...
    if(t === T.LEAF || t === T.TORCH || t === T.GRAVE) return false;
  ...
  function maybeStartSquall(dt,env){
    ...
    if(chance<=0 || Math.random()>=chance*dt) return;
  ...
  function speedAt(x,y,getTile){ return currentSpeed()*exposureAt(x,y,getTile); }
  ...
  function applyToHero(player,dt,getTile,opts){
    ...
    const max = airborne ? MOVE.MAX*2.8 : MOVE.MAX*1.35;
  ...
  function spawnVisualParticle(player,getTile){
    ...
    let tx=Math.floor(bx), ty=Math.floor(by);
    if(!openVisualSpot(tx,ty,getTile)){
      let found=false;
      for(let i=0;i<9;i++){
    ...
    const leafy=Math.random()<clamp(0.22+mag*0.07,0.22,0.62);
    particles.push({
      kind:leafy?'leaf':'dust',
      x:tx+Math.random(),
      y:ty+Math.random(),
      vx:sp*(0.48+Math.random()*0.34) + dir*(0.2+Math.random()*0.35),
      vy:(Math.random()-0.5)*(leafy?0.45:0.20) - mag*0.015,
      life:0,
      max:(leafy?1.2:0.75)+Math.random()*0.85,
      phase:Math.random()*Math.PI*2,
      spin:(Math.random()-0.5)*(leafy?8:3),
      size:leafy?(0.12+Math.random()*0.16):(0.06+Math.random()*0.08),
      color:leafy ? (Math.random()<0.55?'#8fcf4f':(Math.random()<0.5?'#c7a35a':'#6fb35f')) : '#d8c7a2'
    });
    return true;
  }
  function updateVisuals(dt,player,getTile){
    ...
    if(mag>0.04){
      const cap=particleCap();
      const rate=clamp(1.5 + mag*4.5,1.5,22);
      visualAcc += rate*dt;
  ...
  function draw(ctx,TILE,sx,sy,viewX,viewY,canDrawTile){
    ...
    for(const p of particles){
      ...
      if(p.kind==='leaf'){
        ...
        ctx.strokeStyle='rgba(236,226,198,'+a.toFixed(3)+')';
        ctx.lineWidth=Math.max(1,TILE*0.035);
        const len=TILE*(0.26+Math.min(0.42,mag*0.06));
        ctx.beginPath();
  ...
  const api={
    ...
    config:CFG,
    _debug:{particles, squall, computeEnvironment, computeTarget, exposureAt}
  };
  root.MM.wind=api;
})();
