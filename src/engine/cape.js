...
(function(){
  ...
  function draw(ctx,TILE){ if(!cape.length) return; const segs=MM.CAPE.SEGMENTS; const wTop=0.10,wBot=0.24; const left=[],right=[]; for(let i=0;i<segs;i++){ const cur=cape[i]; const next=cape[Math.min(segs-1,i+1)]; let dx=next.x-cur.x, dy=next.y-cur.y; const d=Math.hypot(dx,dy)||1; dx/=d; dy/=d; const t=i/(segs-1); const w=wTop+(wBot-wTop)*t; const px=-dy*w, py=dx*w; left.push({x:cur.x+px,y:cur.y+py}); right.push({x:cur.x-px,y:cur.y-py}); } ctx.fillStyle='#b91818'; ctx.beginPath(); ctx.moveTo(left[0].x*TILE,left[0].y*TILE); for(let i=1;i<left.length;i++) ctx.lineTo(left[i].x*TILE,left[i].y*TILE); for(let i=right.length-1;i>=0;i--) ctx.lineTo(right[i].x*TILE,right[i].y*TILE); ctx.closePath(); ctx.fill(); ctx.strokeStyle='rgba(255,255,255,0.18)'; ctx.lineWidth=1; ctx.beginPath(); for(let i=0;i<left.length;i++) ctx.lineTo(left[i].x*TILE,left[i].y*TILE); ctx.stroke(); }
  capeAPI.init = init;
