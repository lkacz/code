...
import { softDrifts as SOFT_DRIFTS } from './engine/soft_drifts.js';
import { smr as SMR } from './engine/smr.js';
import { avalanche as AVALANCHE } from './engine/avalanche.js';
import { icicles as ICICLES } from './engine/icicles.js';
import { thinIce as THIN_ICE_SIM } from './engine/thin_ice.js';
import { geothermal as GEOTHERMAL } from './engine/geothermal.js';
import { skyMoods as SKY_MOODS } from './engine/sky_moods.js';
import { weatherInstruments as WEATHER_INSTRUMENTS } from './engine/weather_instruments.js';
import { graffiti as GRAFFITI } from './engine/graffiti.js';
...
import VISION_MODES, { VISION_MODES as VISION_MODE_IDS } from './engine/vision_modes.js';
...
MM.treasureScanner=TREASURE_SCANNER;
...
const WORLD_MAX_ABS_X=30e6;
function worldCellInBounds(x,y){
	return Number.isFinite(x) && Math.abs(x)<=WORLD_MAX_ABS_X && worldYInBounds(y);
}
...
function formatAtomicWinterCountdown(value){
	const raw=Number(value);
	const total=Math.max(0,Math.ceil(Number.isFinite(raw)?raw:0));
	const hours=Math.floor(total/3600);
	const minutes=Math.floor((total%3600)/60);
	const seconds=total%60;
	return hours+':'+String(minutes).padStart(2,'0')+':'+String(seconds).padStart(2,'0');
}
let _lastAtomicWinterTimerSecond=-1;
let _lastAtomicWinterTimerActive=false;
function updateAtomicWinterTimerHud(){
	const panel=document.getElementById('atomicWinterTimerPanel');
	const value=document.getElementById('atomicWinterTimerValue');
	if(!panel || !value) return;
	let metrics=null;
	try{ metrics=(ATOMIC_WINTER && ATOMIC_WINTER.metrics) ? ATOMIC_WINTER.metrics() : null; }catch(e){}
	const tLeft=Math.max(0,Number(metrics && metrics.tLeft)||0);
	const active=!!(metrics && metrics.active && tLeft>0);
	if(active!==_lastAtomicWinterTimerActive){
		_lastAtomicWinterTimerActive=active;
		panel.hidden=!active;
		if(!active) _lastAtomicWinterTimerSecond=-1;
	}
	if(!active) return;
	const seconds=Math.ceil(tLeft);
	if(seconds!==_lastAtomicWinterTimerSecond){
		_lastAtomicWinterTimerSecond=seconds;
		const text=formatAtomicWinterCountdown(seconds);
		value.textContent=text;
		panel.title='Zima nuklearna — pozostało '+text;
		panel.setAttribute('aria-label','Zima nuklearna, pozostały czas: '+text);
		const duration=Math.max(0,Number(metrics.duration)||0);
		const progress=Number.isFinite(metrics.progress)
			? Math.max(0,Math.min(1,metrics.progress))
			: (duration>0 ? Math.max(0,Math.min(1,tLeft/duration)) : 0);
		panel.style.setProperty('--atomic-winter-progress',(progress*100).toFixed(2)+'%');
	}
}
...
// Layer mutators may report `false`, or throw from a downstream lifecycle hook
// after the storage write already happened. Inventory follows the observed
// postcondition, never merely the callback return value.
function setInfrastructureConfirmed(x,y,t){
	try{
		if(WORLD && WORLD.setInfrastructure) WORLD.setInfrastructure(x,y,t);
		else setTile(x,y,t);
	}catch(e){}
	return hasInfrastructureTile(x,y,t);
}
function clearInfrastructureConfirmed(x,y,t){
	try{ if(WORLD && WORLD.clearInfrastructure) WORLD.clearInfrastructure(x,y,t); else return false; }catch(e){}
	return !hasInfrastructureTile(x,y,t);
}
function setConstructionBackgroundConfirmed(x,y,t){
	try{ if(WORLD && WORLD.setConstructionBackground) WORLD.setConstructionBackground(x,y,t); else return false; }catch(e){}
	return getConstructionBackgroundTile(x,y)===t;
}
function clearConstructionBackgroundConfirmed(x,y){
	try{ if(WORLD && WORLD.clearConstructionBackground) WORLD.clearConstructionBackground(x,y); else return false; }catch(e){}
	return getConstructionBackgroundTile(x,y)===T.AIR;
}
function setForegroundConfirmed(x,y,t){
	try{ setTile(x,y,t); }catch(e){}
...
function isPowerCableTileId(t){ return !!(INFO[t] && INFO[t].powerCable); }
function getElectricNetworkTile(x,y){
	if(hasInfrastructureTile(x,y,T.SILVER_WIRE)) return T.SILVER_WIRE;
	if(hasInfrastructureTile(x,y,T.COPPER_WIRE)) return T.COPPER_WIRE;
	const network=getNetworkTile(x,y);
	return isPowerCableTileId(network) ? network : getTile(x,y);
}
...
function noteHeroEnergyDelta(delta,opts){
	...
	pushWorldNumber({
		...
		target:opts.target||'hero:energy',
...
function recordTurboEnergyUse(amount){
	const spent=Math.max(0,Number(amount)||0);
	if(!(spent>0) || turboEnergyHoldExhausted) return 0;
	turboEnergyHoldSpent+=spent;
	return spent;
}
function reportTurboEnergyUse(lockUntilRelease){
	const spent=Math.max(0,Number(turboEnergyHoldSpent)||0);
	turboEnergyHoldSpent=0;
	turboEnergyHoldExhausted=!!lockUntilRelease;
	if(!(spent>1e-6)) return 0;
	noteHeroEnergyDelta(-spent,{force:true,cause:'turbo',target:'hero:energy:turbo'});
	return spent;
}
function endTurboEnergyHold(){
	const spent=turboEnergyHoldExhausted ? 0 : reportTurboEnergyUse(false);
	turboEnergyHoldExhausted=false;
	return spent;
}
function spendTurboEnergy(dt){
	...
	applyHeroEnergyCapacity();
	const before=Math.max(0, Number(player.energy)||0);
	if(before<=TURBO_MIN_ENERGY){
		if(turboEnergyHoldSpent>0) reportTurboEnergyUse(true);
		return false;
	}
	...
	if(player.energy<0.0001) player.energy=0;
	recordTurboEnergyUse(spent);
	applyUnderwaterEnergyUseDamage(spent);
	if(player.energy<=TURBO_MIN_ENERGY) reportTurboEnergyUse(true);
	const now=performance.now();
...
function setHeroLampEnabled(value,opts){
	...
	if(res.changed){
		...
		refreshHeroLampButton();
		if(!opts.silent){
			try{ if(AUDIO && AUDIO.play) AUDIO.play('uiClick'); }catch(e){}
...
function updateHeroLamp(dt){
	if(!HERO_LAMP) return;
	// halogen bulb ownership upgrades the eye lamp's beam (tier is derived, not saved)
	if(HERO_LAMP.setTier) HERO_LAMP.setTier((inv.halogen|0)>0?1:0);
...
let visionSaveAt=0;
let visionButtonKey='';
let treasureButtonKey='';
function equippedSpecialVision(){
	try{
		const item=MM.inventory && MM.inventory.equippedItem ? MM.inventory.equippedItem('eyes') : null;
		if(!item || !(Number(item.specialVisionLevel)>0)) return null;
		return {item,level:Math.max(1,Math.min(4,Math.trunc(Number(item.specialVisionLevel)||1))),mode:item.visionMode==='thermal'?'thermal':'night'};
	}catch(e){ return null; }
}
function specialVisionAvailability(){
	const gear=equippedSpecialVision();
	return {
		night:gear && gear.mode==='night' ? gear.level : 0,
		thermal:gear && gear.mode==='thermal' ? gear.level : 0
	};
}
...
function refreshVisionButton(){
	...
	if(!b || !VISION_MODES) return;
	const gear=equippedSpecialVision();
	const info=VISION_MODES.info();
	const on=info.mode!==VISION_MODE_IDS.OFF;
	const shortcut=visionShortcutLabel();
	const key=[gear?gear.mode:'none',gear?gear.level:0,on?info.mode:'off',Math.floor(Math.max(0,Number(player.energy)||0)),shortcut].join('|');
	if(key===visionButtonKey) return;
	visionButtonKey=key;
	b.disabled=!gear;
	b.classList.toggle('on',on);
	b.classList.toggle('thermal',on && info.mode===VISION_MODE_IDS.THERMAL);
	b.setAttribute('aria-pressed',on?'true':'false');
	b.textContent=gear && gear.mode==='thermal' ? '🌡️' : '🌙';
	const modeName=gear && gear.mode==='thermal'?'termowizję':'noktowizję';
	b.title=gear
		? (on?'Wyłącz '+modeName+' ('+shortcut+')':'Włącz '+modeName+' '+gear.level+' ('+shortcut+') · koszt '+VISION_MODES.drainFor(gear.mode,gear.level).toFixed(2)+' E/s')
		: 'Załóż noktowizor lub termowizor w slocie oczu';
	b.setAttribute('aria-label',b.title);
}
function reconcileSpecialVision(){
	if(!VISION_MODES) return {changed:false};
	const res=VISION_MODES.reconcileAvailability(specialVisionAvailability());
	if(res.changed){ visionButtonKey=''; refreshVisionButton(); }
	return res;
}
function toggleSpecialVision(){
	if(!VISION_MODES) return false;
	const gear=equippedSpecialVision();
	if(!gear){ msg('Załóż noktowizor lub termowizor w slocie oczu'); refreshVisionButton(); return false; }
	const next=VISION_MODES.mode()===VISION_MODE_IDS.OFF ? gear.mode : VISION_MODE_IDS.OFF;
	const res=VISION_MODES.setMode(next,gear.level,MM.heroEnergy,{unlimited:godMode});
	if(res.blocked==='energy') msg('Za mało energii, aby uruchomić '+(gear.mode==='thermal'?'termowizję':'noktowizję'));
	else if(res.blocked==='cooldown') msg('Optyka potrzebuje chwili na ponowną kalibrację');
	else if(res.changed){
		msg(next===VISION_MODE_IDS.OFF?'Wizja specjalna wyłączona':(gear.mode==='thermal'?'Termowizja':'Noktowizja')+' '+gear.level+' włączona');
		try{ if(AUDIO && AUDIO.play) AUDIO.play('uiClick'); }catch(e){}
		noteSaveActivity(); saveState();
	}
	visionButtonKey=''; refreshVisionButton();
	return VISION_MODES.isOn();
}
function updateSpecialVision(dt){
	if(!VISION_MODES) return;
	const reconciled=reconcileSpecialVision();
	const res=VISION_MODES.update(dt,MM.heroEnergy,{unlimited:godMode});
	if(res.depleted){ msg('Wizja specjalna zgasła — brak energii'); noteSaveActivity(); saveState(); }
	else if(res.spent>0){
		const now=performance.now();
		if(now-visionSaveAt>2500){ visionSaveAt=now; saveState(); }
	}
	if(reconciled.changed || res.changed) visionButtonKey='';
	refreshVisionButton();
}
function treasureCompassLevel(){
	const raw=MM.activeModifiers && Number(MM.activeModifiers.treasureSenseLevel);
	return Number.isFinite(raw)?Math.max(0,Math.min(4,Math.trunc(raw))):0;
}
function boundedDroppedTreasureTargets(passToken){
	const list=DROPS && DROPS._debug && Array.isArray(DROPS._debug.list) ? DROPS._debug.list : [];
	return TREASURE_DROP_WINDOW.sample(list,passToken,(d,sourceIndex)=>{
		if(!d || d.kind!=='chest' || !Number.isFinite(Number(d.x)) || !Number.isFinite(Number(d.y))) return null;
		return {id:String(d.id||d.lootSeed||('drop_'+sourceIndex)),kind:'chest',tier:d.tier||'common',x:Number(d.x),y:Number(d.y)};
	});
}
function updateTreasureCompass(dt){
	const level=treasureCompassLevel();
	const passToken=TREASURE_SCANNER.metrics().completedPasses;
	const extraTargets=level>0 ? boundedDroppedTreasureTargets(passToken) : [];
	TREASURE_SCANNER.update({dt,level,player,getTile,isDiscovered:worldTileDiscovered,extraTargets});
	refreshTreasureButton();
}
function treasureDirectionLabel(target){
	const d=target && target.direction && target.direction.octant;
	return ({N:'północ',NE:'północny wschód',E:'wschód',SE:'południowy wschód',S:'południe',SW:'południowy zachód',W:'zachód',NW:'północny zachód'})[d]||'nieznany kierunek';
}
function announceTreasureCompass(){
	const level=treasureCompassLevel();
	if(!level){ msg('Załóż wisiorek-kompas, aby wykrywać odkryte skarby'); return false; }
	const target=TREASURE_SCANNER.target();
	if(target) msg('🧭 '+target.label+' · '+Math.max(1,Math.round(target.distance))+' bloków · '+treasureDirectionLabel(target));
	else {
		const metrics=TREASURE_SCANNER.metrics();
		msg(metrics.progress<1?'🧭 Wisiorek stroi się z otoczeniem…':'🧭 Brak odkrytych kosztowności w zasięgu');
	}
	return true;
}
...
window.addEventListener('mm-inventory-change',()=>{
	reconcileSpecialVision();
	visionButtonKey=''; treasureButtonKey='';
...
// Pickaxe-HEAD gear perks (inventory kind 'pickaxe'; the item carries only the
// pickPerk identity — these are the numbers): lucky = ~1 in 10 blocks shatters
// at the first bite, double = extra yield chance, vein = a matching neighbour
// sometimes cracks along (depth-1 chain through the same break chokepoint).
const PICK_PERKS={lucky:{chance:0.10}, double:{chance:0.25}, vein:{chance:0.25}};
function equippedPickaxeItem(){
	try{ return MM.inventory && MM.inventory.equippedItem ? MM.inventory.equippedItem('pickaxe') : null; }catch(e){ return null; }
}
function equippedPickPerk(){ const it=equippedPickaxeItem(); return (it && it.pickPerk)||null; }
...
Object.assign(TILE_LABELS,{
	...
	[T.BEDROCK_LADDER]:'Drabinka macierzysta',
	...
	[T.SILVER_WIRE]:'Przewod srebrny',
	[T.SILVER_ORE]:'Ruda srebra',
	[T.SILVER_INGOT]:'Sztabka srebra',
...
function isFurnishingTileId(t){ return !!(INFO[t] && INFO[t].furniture); }
...
function pumpOrientationLabel(dir=pumpOrientation){
	return dir==='east' ? 'wyjście w prawo'
		: (dir==='west' ? 'wyjście w lewo'
		: (dir==='south' ? 'wyjście w dół' : 'wyjście w górę'));
}
...
function cleanGraveRecord(src){
	...
	for(const key of RESOURCE_KEYS){
		const amount=Math.max(0,Math.floor(Number(src.res[key])||0));
		if(amount>0) res[key]=amount;
	}
...
function restoreGrave(src){
	grave=cleanGraveRecord(src);
	if(grave){
...
function deathTravelSurfaceYAtPoint(target){
	try{
		const x=target && Number.isFinite(Number(target.x)) ? Math.floor(Number(target.x)) : 0;
		const y=WORLDGEN && WORLDGEN.surfaceHeight ? WORLDGEN.surfaceHeight(x) : null;
		return Number.isFinite(y) ? y : null;
	}catch(e){ return null; }
}
function deathTravelRouteConfig(sourceSurfaceY,targetSurfaceY){
	return {
		sectionHeight:worldSectionHeight(),
		baseSectionMin:0,
		baseSectionMax:baseWorldSectionMax(),
		minY:worldMinY(),
		maxY:worldMaxY(),
		clearance:DEATH_TRAVEL_GROUND_CLEARANCE,
		edgeMargin:2,
		sourceSurfaceY:sourceSurfaceY!=null && Number.isFinite(Number(sourceSurfaceY)) ? Number(sourceSurfaceY) : null,
		targetSurfaceY:targetSurfaceY!=null && Number.isFinite(Number(targetSurfaceY)) ? Number(targetSurfaceY) : null,
		surfaceBand:18
	};
}
function lockDeathRespawnTarget(target){
	let spot=target;
	if(!spot || !Number.isFinite(Number(spot.x)) || !Number.isFinite(Number(spot.y)) || !worldYInBounds(Number(spot.y))){
		spot=defaultRespawnTarget();
	}
	return Object.freeze(Object.assign({},spot,{
		x:Number(spot.x),
		y:Number(spot.y),
		kind:String(spot.kind||'spawn')
	}));
}
function deathTravelPointAt(fx,p){
	const t=deathClamp01(p);
	const x=deathLerp(fx.from.x,fx.to.x,t);
	const terrainY=fx.surfaceRoute ? deathTravelGroundYAt(fx,x) : null;
	return sectionAwareRespawnPoint(fx,t,terrainY,deathTravelRouteConfig(fx.sourceSurfaceY,fx.targetSurfaceY));
}
function deathTravelRawGroundYAt(fx,tx){
	if(!fx || fx.surfaceRoute!==true) return null;
	...
	if(!Number.isFinite(y)) y=deathLerp(fx.from.y,fx.to.y,0.5);
	y=Math.max(2,Math.min(WORLD_H-2,y));
	fx.groundCache['r'+tx]=y;
	return y;
}
...
function startDeathTravelFx(cause){
	...
	const to=lockDeathRespawnTarget(deathRespawnTarget());
	const dist=Math.hypot(to.x-from.x,to.y-from.y);
	const seed=((Math.abs(Math.round(from.x*97)) ^ Math.abs(Math.round(from.y*193)) ^ Math.round(performance.now())) >>> 0) || 1;
	const sourceSurfaceY=deathTravelSurfaceYAtPoint(from);
	const targetSurfaceY=deathTravelSurfaceYAtPoint(to);
	const route={
		from,to,dist,t:0,emitT:0,seed,cause:cause||'damage',groundCache:Object.create(null),
		fromSection:worldSectionY(from.y),
		toSection:worldSectionY(to.y),
		sourceSurfaceY,
		targetSurfaceY,
		surfaceRoute:usesSurfaceRespawnRoute(from.y,to.y,deathTravelRouteConfig(sourceSurfaceY,targetSurfaceY))
	};
...
function finishDeathTravelRespawn(){
	...
	if(!placePlayerAtRespawnSpot(fx.to)) placePlayerAtRespawnSpot(lockDeathRespawnTarget(defaultRespawnTarget()));
	// The flight camera followed fx.to exactly. Hand control back only after the
	// physical player and the camera share that same immutable destination.
	centerOnPlayer();
...
// A loaded snapshot replaces the whole world. Drop every side-store marker
// before restoring the snapshot so an older save cannot inherit coordinates
// from the live world merely because its schema lacks a marker field.
...
function decodeRLE(b64,totalLen){ 
	if(!Number.isSafeInteger(totalLen) || totalLen<=0) return null;
	...
	for(let i=0; i<bytes.length; i+=2){
		...
		if(count<=0 || oi+count>totalLen) return null;
		out.fill(v,oi,oi+count);
		oi+=count;
	} 
	return oi===totalLen ? out : null;
}
function decodeRaw(b64){ return _bytesFromB64(b64); }
function validSavedChunkRef(cx,sy){
	if(!Number.isFinite(cx)) return null;
	const hasSection=Number.isFinite(sy);
	const ref=normalizeWorldChunkRef({cx,sy:hasSection?Math.floor(sy):null});
	if(!ref) return null;
	const maxCoord=(WORLD && Number.isFinite(WORLD.maxCoord)) ? WORLD.maxCoord : WORLD_MAX_ABS_X;
	if(Math.abs(ref.cx*CHUNK_W)>maxCoord) return null;
	if(hasSection && (ref.sy<worldMinSection() || ref.sy>worldMaxSection())) return null;
	return ref;
}
function decodeSavedChunk(data,rle,size){
	if(typeof data!=='string' || !Number.isSafeInteger(size) || size<=0 || data.length>size*4+64) return null;
	try{
		const arr=rle ? decodeRLE(data,size) : decodeRaw(data);
...
function restoreTerrainChunk(cx,arr){
	...
	if(!ref) return;
	const expected=CHUNK_W*(ref.base?WORLD_H:worldSectionHeight());
	if(!(arr instanceof Uint8Array) || arr.length!==expected) return;
...
function restoreModifiedChunks(list){
	...
	for(const ch of list){
		...
		if(!ref) continue;
		const size=CHUNK_W*(ref.base?WORLD_H:worldSectionHeight());
		const arr=decodeSavedChunk(ch.data,!!ch.rle,size);
		if(!arr) continue;
		if(ref.base) restoreTerrainChunk(ch.cx,arr); else restoreTerrainChunk(ch.cx,arr,ref.sy);
...
function restoreReferencedChunks(refs){
	...
	for(const saved of refs){
		...
		const size=CHUNK_W*(ref.base?WORLD_H:worldSectionHeight());
		let data=null;
		try{ data=localStorage.getItem(saved.key); }catch(e){ data=null; }
		if(typeof data!=='string' || data.length>size*4+64) continue;
		if(saved.h && computeHash(data)!==saved.h){ console.warn('Autosave chunk hash mismatch',saved.cx); continue; }
		const arr=decodeSavedChunk(data,saved.rle!==false,size);
		if(!arr) continue;
		if(ref.base) restoreTerrainChunk(saved.cx,arr); else restoreTerrainChunk(saved.cx,arr,ref.sy);
		restored.push(ref.base?ref.cx:{cx:ref.cx,sy:ref.sy});
	}
	return restored;
}
...
function snapshotPlayerState(){
	return {
		...
		vision:(VISION_MODES && VISION_MODES.snapshot) ? VISION_MODES.snapshot() : {v:1,mode:'off',level:1}
	};
}
function restorePlayerState(src){
	...
	if(VISION_MODES && VISION_MODES.restore) VISION_MODES.restore(src.vision);
	reconcileSpecialVision();
...
function buildSaveObject(opts){
 ...
 const worldData = timedSavePart(Array.isArray(opts.chunkRefs)?'world.refs':'world.chunks',()=>(
	...
	smoke: timedSavePart('smoke',()=>((SMOKE && SMOKE.snapshot) ? SMOKE.snapshot() : null),perf),
	...
	furnishingsPower: timedSavePart('furnishingsPower',()=>((FURNISHINGS && FURNISHINGS.snapshotPower) ? FURNISHINGS.snapshotPower() : null),perf),
	...
	graffiti: timedSavePart('graffiti',()=>((GRAFFITI && GRAFFITI.snapshot) ? GRAFFITI.snapshot() : null),perf),
...
function applyGameDataCore(data,opts){
 ...
 try{
	...
	const legacyWorldMarkers={
		respawnTotems:!hasOwn('respawnTotems') && sameLiveSeed ? snapshotRespawnTotems() : null,
		healingShelters:!hasOwn('healingShelters') && sameLiveSeed ? snapshotHealingShelters() : null,
		grave:!hasOwn('grave') && sameLiveSeed ? snapshotGrave() : null
	};
	resetWorldTransitionRuntime();
	// Exploration helpers keep incremental cursors/targets outside the world
	// snapshot. Clear them before every restore so a cross-world load can never
	// display a stale treasure bearing or leave special vision enabled when an
	// older save has no player optics state.
	try{ if(TREASURE_SCANNER && TREASURE_SCANNER.reset) TREASURE_SCANNER.reset(); }catch(e){}
	try{ if(VISION_MODES && VISION_MODES.reset) VISION_MODES.reset(); }catch(e){}
	TREASURE_DROP_WINDOW.reset();
	visionButtonKey=''; treasureButtonKey='';
	...
	try{ if(SOFT_DRIFTS && SOFT_DRIFTS.reset) SOFT_DRIFTS.reset(); }catch(e){}
	try{ if(SMR && SMR.reset) SMR.reset(); }catch(e){}
	try{ if(AVALANCHE && AVALANCHE.reset) AVALANCHE.reset(); }catch(e){}
	try{ if(ICICLES && ICICLES.reset) ICICLES.reset(); }catch(e){}
	try{ if(THIN_ICE_SIM && THIN_ICE_SIM.reset) THIN_ICE_SIM.reset(); }catch(e){}
	try{ if(GEOTHERMAL && GEOTHERMAL.reset) GEOTHERMAL.reset(); }catch(e){}
	try{ if(SKY_MOODS && SKY_MOODS.reset) SKY_MOODS.reset(); }catch(e){}
	try{ if(WEATHER_INSTRUMENTS && WEATHER_INSTRUMENTS.reset) WEATHER_INSTRUMENTS.reset(); }catch(e){}
	try{ if(GRAFFITI && GRAFFITI.reset) GRAFFITI.reset(); }catch(e){}
	...
	try{ if(SMOKE && SMOKE.reset) SMOKE.reset(); }catch(e){}
...
let craftedOutputFailed=false;
function grantCraftedItem(def){
	def.id='crafted_'+def.kind+'_'+Date.now().toString(36)+Math.random().toString(36).slice(2,5);
	// Use the inventory's capacity-aware grant path first. Returning false lets
	// doCraft roll the material transaction back instead of claiming an item was
	// equipped while it only remained stranded in dynamicLoot.
	if(MM.inventory && typeof MM.inventory.grantItem==='function'){
		const granted=MM.inventory.grantItem(def,{equip:true,markNew:false});
		if(!granted){ craftedOutputFailed=true; msg('Plecak jest pełny — zwolnij miejsce przed wytworzeniem '+def.name); return false; }
...
const RECIPES=[
	...
	{id:'bedrock_ladders', name:'Drabinki macierzyste x6', cost:{bedrock:1}, make(){ inv.bedrockLadder+=6; msg('Drabinki macierzyste +6 - jeden zaczep u gory lub u dolu wystarczy na dowolna dlugosc'); }},
	...
	// Every stone type knaps into throwing rocks: harder rock hits harder and
	// survives the impact more often (weapons.js STONE_TIERS is the ladder).
	{id:'throwing_stones_granite', name:'Kamienie granitowe x6', cost:{granite:2}, make(){ inv.throwingStoneGranite+=6; msg('Kamienie granitowe +6 - mocniejszy rzut, czesciej przezywa upadek'); }},
	...
	// Dzidy (fireRange:3) oddają część obrażeń za długi, wyłącznie poziomy zasięg; miecze to szczyt materiału.
	...
	{id:'spear_metal', name:'Dzida metalowa', cost:{wood:3, steel:2}, make(){ grantCraftedItem({kind:'weapon',weaponType:'melee',name:'Dzida metalowa',attackDamage:5,fireRange:3,meleeEffect:'bleed',tier:'rare',desc:'Poziome pchnięcie na trzy pola; ładowany stalowy grot tnie do krwi'}); }},
	{id:'spear_diamond', name:'Dzida diamentowa', cost:{wood:3, diamond:2}, make(){ grantCraftedItem({kind:'weapon',weaponType:'melee',name:'Dzida diamentowa',attackDamage:7,fireRange:3,meleeEffect:'panic',tier:'epic',desc:'Ładowane poziome pchnięcie na trzy pola — wrogowie pierzchają'}); }},
	...
	{id:'loot_charm_tile', name:'Sznurek zbieracza (własny blok)', cost:{wood:1, leaf:2}, make(){ grantCraftedItem({kind:'charm',name:'Sznurek zbieracza',lootMagnetLevel:1,tier:'common',desc:'Automatycznie przyciąga łupy z bloku, w którym stoisz'}); }},
	{id:'loot_charm_copper', name:'Miedziany magnes łupów (+1)', cost:{copper:2, plastic:1}, make(){ grantCraftedItem({kind:'charm',name:'Miedziany magnes łupów',lootMagnetLevel:2,tier:'uncommon',desc:'Automatycznie przyciąga łupy z promienia jednego bloku'}); }},
	{id:'loot_charm_silver', name:'Srebrny magnes łupów (+2)', cost:{silver:3, plastic:1, meteorDust:1}, make(){ grantCraftedItem({kind:'charm',name:'Srebrny magnes łupów',lootMagnetLevel:3,tier:'rare',desc:'Automatycznie przyciąga łupy z promienia dwóch bloków'}); }},
	{id:'loot_charm_antimatter', name:'Węzeł antymaterii łupów (+3)', cost:{iridium:2, antimatter:1, transistor:2}, make(){ grantCraftedItem({kind:'charm',name:'Węzeł antymaterii łupów',lootMagnetLevel:4,tier:'epic',desc:'Automatycznie przyciąga łupy z promienia trzech bloków'}); }},
	{id:'treasure_compass_copper', name:'Miedziany kompas poszukiwacza I', cost:{copper:2, glass:1, wood:1}, make(){ grantCraftedItem({kind:'charm',name:'Miedziany kompas poszukiwacza',treasureSenseLevel:1,tier:'common',desc:'Wisiorek wychyla się ku odkrytym rudom i skrzyniom w promieniu 12 bloków'}); }},
	{id:'treasure_compass_silver', name:'Srebrny kompas poszukiwacza II', cost:{silver:2, glass:1, transistor:1}, make(){ grantCraftedItem({kind:'charm',name:'Srebrny kompas poszukiwacza',treasureSenseLevel:2,tier:'uncommon',desc:'Precyzyjny wisiorek wskazuje odkryte kosztowności do 22 bloków'}); }},
	{id:'treasure_compass_iridium', name:'Irydowy kompas poszukiwacza III', cost:{iridium:2, silverWire:2, meteorDust:1}, make(){ grantCraftedItem({kind:'charm',name:'Irydowy kompas poszukiwacza',treasureSenseLevel:3,tier:'rare',desc:'Wykrywa odkryte rudy, relikty i skrytki do 36 bloków'}); }},
	{id:'treasure_compass_antimatter', name:'Kompas antymaterii IV', cost:{antimatter:1, iridium:2, transistor:2}, make(){ grantCraftedItem({kind:'charm',name:'Kompas antymaterii',treasureSenseLevel:4,tier:'epic',desc:'Najczulszy wisiorek skanuje odkryte kosztowności do 52 bloków'}); }},
	{id:'night_goggles_basic', name:'Gogle noktowizyjne I', cost:{glass:2, copperWire:1, glowshroom:2}, make(){ grantCraftedItem({kind:'eyes',name:'Gogle fosforowe',specialVisionLevel:1,visionMode:'night',tier:'common',desc:'Zielona noktowizja wzmacnia zastane światło; włączana skrótem wizji'}); }},
	{id:'night_goggles_silver', name:'Gogle noktowizyjne II', cost:{glass:3, silverWire:2, transistor:1}, make(){ grantCraftedItem({kind:'eyes',name:'Srebrna optyka nocna',specialVisionLevel:2,visionMode:'night',tier:'uncommon',desc:'Czystszy obraz nocny o umiarkowanym zużyciu energii; włączany skrótem wizji'}); }},
	{id:'thermal_goggles_iridium', name:'Termowizor irydowy III', cost:{glass:3, silverWire:2, radioactiveOre:1, transistor:2}, make(){ grantCraftedItem({kind:'eyes',name:'Irydowy termowizor',specialVisionLevel:3,visionMode:'thermal',tier:'rare',desc:'Wyróżnia ciepłe sylwetki tylko w aktualnej linii widzenia; użyj skrótu wizji'}); }},
	{id:'thermal_goggles_antimatter', name:'Termowizor antymaterii IV', cost:{glass:4, iridium:2, antimatter:1, transistor:3}, make(){ grantCraftedItem({kind:'eyes',name:'Termowizor antymaterii',specialVisionLevel:4,visionMode:'thermal',tier:'epic',desc:'Daleka termowizja bez widzenia przez ściany i mgłę; użyj skrótu wizji'}); }},
	...
	{id:'silver_wire', name:'Przewod srebrny x4', cost:{silver:2, plastic:1}, make(){ inv.silverWire+=4; msg('Przewod srebrny +4 - dwukrotnie wydajniejsze zasilanie bez strat cieplnych'); }},
	{id:'solar_panel', name:'Panel sloneczny x2', cost:{glass:3, wire:3, copperWire:1}, make(){ inv.solarPanel+=2; msg('Panel sloneczny +2 - pracuje caly dzien, najwydajniej w poludnie; lokalne chmury oslabiaja moc'); }},
	{id:'solar_battery', name:'Panel sloneczny z bateria', cost:{solarPanel:2, transistor:1, copperWire:2}, make(){ inv.solarBattery+=1; msg('Panel sloneczny z bateria +1 - zachowuje dzienny uzysk, ale najlepiej dziala w miksie z dynamem'); }},
	{id:'smr_cell', name:'Ogniwo nuklearne SMR', cost:{graphite:4, radioactiveOre:2, transistor:3, meteoricIron:2, silverWire:4}, make(){ inv.smrCell+=1; msg('Ogniwo SMR +1 - najmniejsza, ale wieczna produkcja energii; wymaga obecnosci i okresowej kontroli (E)'); }},
	{id:'soot_pigment', name:'Sadza (pigment)', cost:{coal:1}, make(){ inv.soot=(inv.soot|0)+4; msg('Sadza +4 - pigment do znakow na scianach (G maluje, Shift+G zmienia szablon)'); }},
	{id:'weathervane', name:'Wiatrowskaz', cost:{steel:1, wood:2}, make(){ inv.weathervane=(inv.weathervane|0)+1; msg('Wiatrowskaz +1 - postaw na dachu i czytaj zywy wiatr'); }},
	{id:'lightning_rod', name:'Piorunochron', cost:{steel:3, silverWire:2}, make(){ inv.lightningRod=(inv.lightningRod|0)+1; msg('Piorunochron +1 - sciaga pioruny, bankuje ladunek i oslania stojacych obok'); }},
	{id:'graphene_exoplate', name:'Grafenowy egzoszkielet', cost:{graphene:3, steel:2}, make(){ grantCraftedItem({kind:'outfit',name:'Grafenowy egzoszkielet',crushResistBonus:3,tier:'rare',desc:'Najtwardszy material swiata w formie pancerza — przygniecenia bola o wiele mniej'}); }},
	...
	{id:'potion_depths', name:'Eliksir głębin', cost:{goldenFish:1, water:2}, make(){ if(MM.progress && MM.progress.addBuff) MM.progress.addBuff({stackKey:'potion_depths',name:'Głębiny', icon:'🐠', dur:90, stats:{waterMoveSpeedMult:1.25, jumpPowerMult:1.2}}); msg('🐠 Eliksir głębin: ruch w wodzie 125%, skok +20% (90 s, maks. ×3)'); try{ if(MM.audio && MM.audio.play) MM.audio.play('heal'); }catch(e){} }},
	...
	{id:'potion_speed', name:'Mikstura szybkości', cost:{water:1, leaf:2, diamond:1}, make(){ if(MM.progress && MM.progress.addBuff) MM.progress.addBuff({stackKey:'potion_speed',name:'Szybkość', icon:'💨', dur:60, stats:{moveSpeedMult:1.3, jumpPowerMult:1.15}}); msg('💨 Szybkość +30%, skok +15% (60 s, maks. ×3)'); try{ if(MM.audio && MM.audio.play) MM.audio.play('heal'); }catch(e){} }},
	{id:'potion_strength', name:'Mikstura siły', cost:{water:1, obsidian:1}, make(){ if(MM.progress && MM.progress.addBuff) MM.progress.addBuff({stackKey:'potion_strength',name:'Siła', icon:'💪', dur:60, stats:{attackDamage:5}}); msg('💪 Obrażenia +5 (60 s, maks. ×3)'); try{ if(MM.audio && MM.audio.play) MM.audio.play('heal'); }catch(e){} }},
	...
	{id:'potion_antigrav', name:'Mikstura antygrawitacji', cost:{antimatter:1, water:1}, make(){ if(MM.progress && MM.progress.addBuff) MM.progress.addBuff({stackKey:'potion_antigrav',name:'Antygrawitacja', icon:'🛸', dur:60, stats:{jumpPowerMult:1.6, moveSpeedMult:1.15}}); msg('🛸 Antygrawitacja: skok +60%, ruch +15% (60 s, maks. ×3)'); try{ if(MM.audio && MM.audio.play) MM.audio.play('heal'); }catch(e){} }},
];
...
const CRAFT_RECIPE_META={
	...
	bedrock_ladders:{group:'building',icon:'🪜',out:'bedrockLadder',amount:6,tint:'#8d98b1',desc:'Szczeble ze skaly macierzystej: wymagaja jednego zaczepu u gory lub u dolu, potem moga biec bez konca.'},
	...
	spear_metal:{group:'weapons',icon:'🔱',tint:'#9aa8b5',desc:'Poziome pchniecie na trzy pola; stalowy grot tnie do krwi.'},
	spear_diamond:{group:'weapons',icon:'🔱',tint:'#3ef',desc:'Ladowane poziome pchniecie na trzy pola; diament sieje panike.'},
	...
	treasure_compass_copper:{group:'relics',icon:'🧭',tint:'#c88447',desc:'Poziom I: fizyczny wisiorek i HUD wskazują odkryte skarby do 12 bloków.'},
	treasure_compass_silver:{group:'relics',icon:'🧭',tint:'#dce8f5',desc:'Poziom II: czulszy kompas odkrytych rud i skrzyń, zasięg 22.'},
	treasure_compass_iridium:{group:'relics',icon:'🧭',tint:'#c49bff',desc:'Poziom III: rudy, relikty i skrytki do 36 bloków.'},
	treasure_compass_antimatter:{group:'relics',icon:'🧭',tint:'#ff75ec',desc:'Poziom IV: maksymalny bezpieczny zasięg 52; nie przebija mgły odkrycia.'},
	night_goggles_basic:{group:'relics',icon:'🌙',tint:'#70ef91',desc:'Noktowizja I: zielony fosfor, delikatne ziarno i niski koszt energii; włącza ją skrót wizji.'},
	night_goggles_silver:{group:'relics',icon:'🌙',tint:'#a8ffc0',desc:'Noktowizja II: jaśniejszy i czystszy obraz; nie odsłania ścian ani mgły.'},
	thermal_goggles_iridium:{group:'relics',icon:'🌡️',tint:'#ff8a48',desc:'Termowizja III: widoczne istoty stają się ciepłymi sylwetkami, teren pozostaje chłodny.'},
	thermal_goggles_antimatter:{group:'relics',icon:'🌡️',tint:'#ff5f7e',desc:'Termowizja IV: najdalszy zasięg i najwyższy koszt; nadal bez widzenia przez ściany.'},
	...
	silver_wire:{group:'machines',icon:'⚡',out:'silverWire',amount:4,desc:'Przewodzi bez strat; z tego samego źródła dostarcza urządzeniom dwa razy więcej energii niż miedź.'},
	solar_panel:{group:'machines',icon:'🔆',out:'solarPanel',amount:2,desc:'Uzupelniajace zrodlo: malo o swicie, maksimum w poludnie; cien lokalnych chmur plynnie obniza moc.'},
	smr_cell:{group:'machines',icon:'☢',out:'smrCell',amount:1,desc:'Najmniejsza, ale wieczna produkcja energii. Pracuje tylko przy obecnosci druzyny i prosi o okresowa kontrole; w wodzie gotuje pare do zamknietego obiegu.'},
	soot_pigment:{group:'survival',icon:'🖌',out:'soot',amount:4,desc:'Rozkruszony wegiel na czarny pigment. G maluje znak na scianie (strzalka/X/serce/kropka), Shift+G zmienia szablon — drogowskazy dla druzyny.'},
	throwing_stones_granite:{group:'weapons',icon:'🪨',out:'throwingStoneGranite',amount:6,desc:'Granit dobrany do dloni: mocniejsze trafienie i wieksza szansa, ze kamien przetrwa i da sie go podniesc.'},
	...
	graphene_exoplate:{group:'relics',icon:'🛡',desc:'Prasowany wegiel po drodze przez grafit i grafen: pancerz, ktory lamie sie ostatni.'},
	solar_battery:{group:'machines',icon:'🔋',out:'solarBattery',amount:1,desc:'Zachowuje dzienny uzysk na noc; sam nie zastapi stabilnego dynama ani elektrowni wodnej.'},
	...
	chair_wood:{group:'furniture',icon:'🪑',out:'chairWood',amount:1,home:{tier:1,bonus:.5,category:'furniture'},desc:'Zwykly mebel: w oswietlonym domu przyspiesza leczenie. Postawiony na maszynie z gasienicami staje sie fotelem pilota (drewno slabo przenosi energie bohatera).'},
	chair_stone:{group:'furniture',icon:'🪑',out:'chairStone',amount:1,home:{tier:1,bonus:.5,category:'furniture'},desc:'Trwalszy mebel do domu i kabiny mecha; lepiej przenosi energie bohatera.'},
	chair_steel:{group:'furniture',icon:'🪑',out:'chairSteel',amount:1,home:{tier:2,bonus:.5,category:'furniture'},desc:'Najtrwalszy fotel: komfort w domu, a jako fotel pilota pelne przeniesienie energii bohatera na naped.'},
	water_pipe:{group:'machines',icon:'🚰',out:'waterPipe',amount:6,desc:'Prowadzi wodę, parę i gazy. PPM przełącza zwykłą rurę i wlot wody.'},
	...
	potion_depths:{group:'alchemy',icon:'🐠',tint:'#ffd76a',desc:'Dar złotej rybki: szybkie pływanie i mocniejszy skok. Maks. 3 warstwy.'},
	potion_heal:{group:'alchemy',icon:'🧪',tint:'#ff5a5a',desc:'Natychmiastowe leczenie.'},
	potion_speed:{group:'alchemy',icon:'💨',tint:'#a8d7ff',desc:'Czasowy buff ruchu i skoku. Maks. 3 warstwy; kolejna odnawia czas.'},
	potion_strength:{group:'alchemy',icon:'💪',tint:'#ffb84a',desc:'Czasowy buff obrażeń. Maks. 3 warstwy; kolejna odnawia czas.'},
	boss_horn:{group:'relics',icon:'📯',tint:'#e0b341',desc:'Dobrowolne wzywanie klopotow, kiedy nagroda kusi za mocno.'},
	potion_antigrav:{group:'alchemy',icon:'🎈',tint:'#c46bff',desc:'Czasowy buff skoku i lekkiego ruchu. Maks. 3 warstwy.'}
};
...
function doCraft(r,count){
	...
	for(let i=0;i<target;i++){
		...
		craftedOutputFailed=false;
		const madeOk=r.make();
		if(madeOk===false || craftedOutputFailed){ for(const k in r.cost) inv[k]+=r.cost[k]; break; }
		made++;
...
function recipeSearchText(r){
	const home=recipeHomeMeta(r);
	const parts=[r.id,r.name,recipeOutputName(r),CRAFT_GROUP_LABELS[recipeGroup(r)]||'',r.desc||'',home&&home.category||'',home&&home.tierLabel||''];
...
function craftSourceHint(k){
	if(CRAFT_SOURCE_HINTS[k]) return CRAFT_SOURCE_HINTS[k];
	const furnishing=FURNISHINGS && FURNISHINGS.byKey && FURNISHINGS.byKey.get(k);
	if(furnishing) return 'craft: wcześniejszy projekt z działu '+(CRAFT_GROUP_LABELS[furnishing.category]||'Wyposażenie');
	return null;
}
function makeCraftChip(k,need,compact){
	...
	const hint=have<need ? craftSourceHint(k) : null;
...
function makeIngredientRow(k,need){
	...
	const sourceHint=craftSourceHint(k);
	if(!ok && sourceHint){
		const hint=document.createElement('div'); hint.className='craftIngHint'; hint.textContent='↳ '+sourceHint;
...
const HOME_TIER_LABELS={1:'Rękodzieło',2:'Domowy luksus',3:'Zaawansowana technika',4:'Cud przyszłości'};
function recipeHomeMeta(r){
	if(!r) return null;
	const byKey=FURNISHINGS && FURNISHINGS.byKey;
	const byTile=FURNISHINGS && FURNISHINGS.byTile;
	const def=(byTile && Number.isFinite(Number(r.tile)) && byTile.get(Number(r.tile))) || (byKey && r.out && byKey.get(r.out)) || null;
	if(!def && !r.home) return null;
	let tile=def && def.tile;
	if(tile==null && r.out){
		const resource=RESOURCE_DEFS.find(item=>item.key===r.out && item.tile);
		if(resource && T[resource.tile]!=null) tile=T[resource.tile];
	}
	const tier=Math.max(1,Math.min(4,Number((def&&def.tier)||(r.home&&r.home.tier)||r.tier)||1));
	const bonus=Math.max(0,Number((def&&def.homeRegenBonus)||(r.home&&r.home.bonus)||r.homeRegenBonus)||0);
	const category=(def&&def.category)||(r.home&&r.home.category)||recipeGroup(r);
	return {def,tile,tier,bonus,category,tierLabel:HOME_TIER_LABELS[tier]||('Poziom '+tier)};
}
function drawCraftHomePreview(canvas,home,opts){
	if(!canvas || !home) return false;
	if(home.def && FURNISHINGS && FURNISHINGS.drawPreview){
		return FURNISHINGS.drawPreview(canvas,home.def,Object.assign({time:performance.now()},opts||{}));
	}
	if(home.tile==null) return false;
	const g=canvas.getContext && canvas.getContext('2d'); if(!g) return false;
	g.clearRect(0,0,canvas.width,canvas.height);
	g.imageSmoothingEnabled=false;
	const scale=Math.min(canvas.width,canvas.height)/(TILE*1.45);
	g.save();
	g.translate((canvas.width-TILE*scale)*.5,(canvas.height-TILE*scale)*.5);
	g.scale(scale,scale);
	drawEntityTile(g,home.tile,0,0,0,0,{preview:true});
	g.restore();
	return true;
}
function renderCraftDetail(r){
	...
	const home=recipeHomeMeta(r);
	detail.classList.toggle('homeDetail',!!home);
	if(home) detail.dataset.tier=String(home.tier); else delete detail.dataset.tier;
	if(!r){
	...
	if(home){
		const hero=document.createElement('div'); hero.className='craftHomeHero'; hero.dataset.tier=String(home.tier);
		const canvas=document.createElement('canvas'); canvas.className='craftHomeCanvas'; canvas.width=112; canvas.height=112;
		canvas.setAttribute('aria-label','Podgląd: '+r.name); hero.appendChild(canvas);
		const copy=document.createElement('div'); copy.className='craftHomeCopy';
		const eyebrow=document.createElement('span'); eyebrow.className='craftHomeEyebrow'; eyebrow.textContent=(CRAFT_GROUP_LABELS[recipeGroup(r)]||'Wyposażenie')+' · kolekcja domowa';
		const tier=document.createElement('span'); tier.className='craftHomeTier'; tier.dataset.tier=String(home.tier); tier.textContent=home.tierLabel;
		const note=document.createElement('span'); note.className='craftDesc';
		const placementHints={floor:'Montaż: wyłącznie na podłodze. ',floor_or_table:'Montaż: na podłodze lub bezpośrednio na stole. ',floor_or_ceiling:'Montaż: na podłodze albo pod sufitem. ',wall:'Montaż: wyłącznie na właściwej ścianie w tle. '};
		note.textContent=(placementHints[home.def && home.def.placement]||'')+
			(home.def && home.def.requiresPower ? 'Wymaga zasilania przewodem, dynamem lub panelem słonecznym. ' : '')+
			(home.def && home.def.lightLevel ? 'Emituje światło '+home.def.lightLevel+'/15. ' : '')+'Postaw w zamkniętym, oświetlonym domu, aby zwiększyć tempo regeneracji.'+
			(home.def && home.def.tile===T.RADIO ? ' Po postawieniu użyj E lub kliknij radio, aby wybrać jedną z sześciu stacji.' : '');
		copy.appendChild(eyebrow); copy.appendChild(tier); copy.appendChild(note); hero.appendChild(copy); detail.appendChild(hero);
		drawCraftHomePreview(canvas,home,{large:true});
		const stats=document.createElement('div'); stats.className='craftHomeStats';
		const bonus=document.createElement('div'); bonus.className='craftHomeStat';
		const bonusVal=document.createElement('strong'); bonusVal.textContent='+'+home.bonus.toFixed(2);
		const bonusLab=document.createElement('small'); bonusLab.textContent='potencjału regeneracji';
		const meter=document.createElement('span'); meter.className='craftHomeMeter'; const meterFill=document.createElement('i'); meterFill.style.width=Math.round(Math.min(1,home.bonus/.36)*100)+'%'; meter.appendChild(meterFill);
		bonus.appendChild(bonusVal); bonus.appendChild(bonusLab); bonus.appendChild(meter);
		const cap=document.createElement('div'); cap.className='craftHomeStat';
		const capVal=document.createElement('strong'); capVal.textContent='×3.00';
		const capLab=document.createElement('small'); capLab.textContent='miękki limit komfortu domu';
		cap.appendChild(capVal); cap.appendChild(capLab); stats.appendChild(bonus); stats.appendChild(cap); detail.appendChild(stats);
	}
...
function makeCraftRow(r){
	...
	const home=recipeHomeMeta(r);
	if(home){ row.classList.add('homeRecipe'); row.dataset.tier=String(home.tier); }
	row.tabIndex=0;
	...
	if(home && home.def){
		const thumb=document.createElement('canvas'); thumb.className='craftTileThumb'; thumb.dataset.tier=String(home.tier); thumb.width=48; thumb.height=48;
		thumb.setAttribute('aria-label','Miniatura '+r.name); row.appendChild(thumb); drawCraftHomePreview(thumb,home,{compact:true});
	}else{
...
function drawPlayer(opts){ if(drawDeathTravelFx()) return; const rearView=!!(opts&&opts.rearView); const remoteBody=!!(opts&&opts.remoteBody); const c=MM.customization||{}; let bodyX=(player.x-player.w/2)*TILE; let bodyY=(player.y-player.h/2)*TILE; let bw=player.w*TILE, bh=player.h*TILE;
	...
	const style = ((c && c.outfitStyle)!=null ? String(c.outfitStyle) : 'default').trim().toLowerCase();
		 ...
		 if(MM && typeof MM.drawOutfit === 'function'){
			 MM.drawOutfit(ctx, bodyX, bodyY, bw, bh, style, c, rearView?{back:true}:null);
		 } else {
	...
	if(rearView){
		// Back-facing pose used while the hero looks into a wall mirror. A collar,
		// shoulder shade and central seam make the orientation readable even for
		// the plain outfit, without inventing a second disconnected sprite set.
		ctx.save();
		ctx.fillStyle=style==='ninja'?'rgba(113,126,164,.55)':(style==='ironperson'?'rgba(255,207,91,.42)':'rgba(74,45,24,.24)');
		ctx.fillRect(bodyX+bw*.12,bodyY+bh*.16,bw*.76,Math.max(1,bh*.08));
		ctx.fillStyle=style==='mystic'?'rgba(219,195,255,.42)':'rgba(38,25,18,.28)';
		ctx.fillRect(bodyX+bw*.47,bodyY+bh*.24,Math.max(1,bw*.06),bh*.56);
		ctx.fillStyle='rgba(255,255,255,.10)';
		ctx.fillRect(bodyX+bw*.18,bodyY+bh*.27,bw*.08,bh*.42);
		ctx.restore();
	}
...
function homeMirrorHasLineOfSight(mx,my){
	...
	const fromX=mx+.5, fromY=my+.5;
	const dx=player.x-fromX, dy=player.y-fromY;
	const distance=Math.hypot(dx,dy);
	if(distance>HOME_MIRROR_RANGE) return false;
	const steps=Math.max(1,Math.ceil(distance*4));
	for(let i=1;i<steps;i++){
		const x=Math.floor(fromX+dx*(i/steps));
		const y=Math.floor(fromY+dy*(i/steps));
		let tile;
		try{ tile=getTile(x,y); }catch(e){ return false; }
		if(tile!==T.MIRROR && tile!==T.GLASS && !isHeroPassableTile(tile)) return false;
	}
	return true;
}

// A real second render of the current hero is cheaper and more accurate than a
// stale sprite snapshot: customization, blink/facial state, recoil, cape cloth
// and the active weapon all stay synchronized. The negative X scale provides
// the optical left/right reversal, while the catalogue pass clips it to glass.
function drawHomeMirrorReflection(g,mirror){
	if(deathTravelFx || !g || !mirror || !Number.isFinite(mirror.distance)
		|| !Number.isFinite(mirror.glassW) || !Number.isFinite(mirror.glassH)
		|| mirror.glassW<=0 || mirror.glassH<=0 || mirror.distance>HOME_MIRROR_RANGE) return false;
	if(!homeMirrorHasLineOfSight(mirror.x,mirror.y)) return false;
	const projection=mirror.projection || (FURNISHINGS && FURNISHINGS.mirrorSubjectProjection
		? FURNISHINGS.mirrorSubjectProjection(player,mirror) : null);
	if(!projection) return false;
	const scale=projection.scale;
	g.save();
	try{
		g.globalAlpha=.80;
		g.translate(projection.centerX,projection.centerY);
		g.scale(-scale,scale);
		g.translate(-player.x*TILE,-player.y*TILE);
		drawCape();
		drawPlayer({rearView:false,mirrorReflection:true});
		if(WEAPONS && WEAPONS.drawHeroReflection) WEAPONS.drawHeroReflection(g,TILE,player);
		if(WEAPONS && WEAPONS.drawHeld) WEAPONS.drawHeld(g,TILE,player);
	}finally{
		g.restore();
	}
	return true;
...
function tileShadeAmp(t){
	...
	if(t===T.GOLD_ORE || t===T.SILVER_ORE) return 0;
	if(t===T.SILVER_INGOT) return 3;
	...
	if(t===T.GRAPHITE) return 4;
	if(t===T.GRAPHENE) return 3;
	if(t===T.SMR_CELL) return 3;
...
function tileEdgeFamily(t){
	switch(t){
		case T.STONE: case T.GRANITE: case T.BASALT: case T.BEDROCK: case T.COAL: case T.GOLD_ORE: case T.SILVER_ORE: case T.SILVER_INGOT: case T.DIAMOND:
		...
		case T.GRAPHITE: return EDGE_ROCK;
		case T.LAVA: case T.MOTHER_LAVA: return EDGE_LAVA;
		default: return 0;
	}
}
...
function drawSilverOreArt(g,px,py,h){
	// Silver is cooler and more angular than gold: paired pale seams with a
	// blue-white specular edge keep it readable inside grey host rock.
	const y0=py+7+((h>>>4)&3), y1=py+12-((h>>>8)&3);
	g.save();
	g.lineCap='round'; g.lineJoin='round';
	g.strokeStyle='rgba(34,45,58,.55)'; g.lineWidth=4;
	g.beginPath(); g.moveTo(px+2,y0); g.lineTo(px+7,y0-3); g.lineTo(px+12,y1); g.lineTo(px+18,y1-4); g.stroke();
	g.strokeStyle='rgba(158,174,192,.96)'; g.lineWidth=2.4;
	g.beginPath(); g.moveTo(px+2,y0); g.lineTo(px+7,y0-3); g.lineTo(px+12,y1); g.lineTo(px+18,y1-4); g.stroke();
	g.strokeStyle='rgba(235,247,255,.92)'; g.lineWidth=1;
	g.beginPath(); g.moveTo(px+3,y0-1); g.lineTo(px+7,y0-4); g.lineTo(px+12,y1-1); g.lineTo(px+17,y1-5); g.stroke();
	g.fillStyle='rgba(255,255,255,.94)';
	const gx=px+6+((h>>>12)&7), gy=py+4+((h>>>16)&7);
	g.fillRect(gx-2,gy,5,1); g.fillRect(gx,gy-2,1,5);
	g.restore();
}
function drawSilverIngotArt(g,px,py,h){
	// Canvas2D has no PBR reflections, so polished silver is sold by contrast:
	// a dark reflected-ground band, a cool horizon and a narrow sky-white streak.
	// Every operation is baked into the chunk canvas (no gradients or blur), making
	// the steady-state cost of this richer material effectively zero.
	const shift=((h>>>5)&3)-1;
	g.fillStyle='rgba(52,70,91,0.34)';
	g.fillRect(px+1,py+TILE-7,TILE-2,5);
	g.fillStyle='rgba(112,143,172,0.34)';
	g.fillRect(px+1,py+9,TILE-2,3);
	g.fillStyle='rgba(206,229,246,0.62)';
	g.fillRect(px+2,py+6,TILE-4,2);
	g.fillStyle='rgba(255,255,255,0.88)';
	g.beginPath();
	g.moveTo(px+1,py+9+shift);
	g.lineTo(px+TILE-1,py+3+shift);
	g.lineTo(px+TILE-1,py+5+shift);
	g.lineTo(px+1,py+11+shift);
	g.closePath();
	g.fill();
	drawBlockBevel(g,px,py,'rgba(255,255,255,0.76)','rgba(43,58,76,0.52)');
	g.strokeStyle='rgba(74,94,116,0.58)';
	g.lineWidth=1;
	g.strokeRect(px+1.5,py+1.5,TILE-3,TILE-3);
	const gx=px+5+((h>>>10)&7), gy=py+4+((h>>>14)&5);
...
// High-value and story materials need an identity at a glance. These details
// are shared by chunk baking and every inventory/build preview, so a rare block
// never falls back to INFO.color's anonymous square in one of the UI surfaces.
function drawMotherIceArt(g,px,py,h){
	g.save();
	// ancient blue ice around a pale, faceted "heart"
	g.fillStyle='rgba(7,67,100,0.22)';
	g.beginPath(); g.moveTo(px+1,py+15); g.lineTo(px+6,py+3); g.lineTo(px+11,py+7); g.lineTo(px+17,py+1); g.lineTo(px+19,py+12); g.lineTo(px+13,py+19); g.closePath(); g.fill();
	g.strokeStyle='rgba(91,205,241,0.46)'; g.lineWidth=1;
	g.beginPath();
	g.moveTo(px+2,py+15); g.lineTo(px+7,py+10); g.lineTo(px+5,py+4);
	g.moveTo(px+7,py+10); g.lineTo(px+11,py+6); g.lineTo(px+16,py+3);
	g.moveTo(px+7,py+10); g.lineTo(px+12,py+13); g.lineTo(px+15,py+18);
	g.moveTo(px+12,py+13); g.lineTo(px+18,py+10);
	g.stroke();
	const ox=((h>>>7)&1), oy=((h>>>10)&1);
	g.fillStyle='rgba(221,253,255,0.88)';
	g.beginPath(); g.moveTo(px+10+ox,py+4+oy); g.lineTo(px+15+ox,py+9+oy); g.lineTo(px+10+ox,py+16+oy); g.lineTo(px+6+ox,py+10+oy); g.closePath(); g.fill();
	g.fillStyle='rgba(103,211,242,0.62)';
	g.beginPath(); g.moveTo(px+10+ox,py+4+oy); g.lineTo(px+10+ox,py+16+oy); g.lineTo(px+6+ox,py+10+oy); g.closePath(); g.fill();
	g.fillStyle='rgba(255,255,255,0.96)'; g.fillRect(px+9+ox,py+6+oy,2,4);
	g.fillStyle='rgba(255,255,255,0.70)';
	g.fillRect(px+3+((h>>>4)&2),py+5,1,1); g.fillRect(px+16,py+13+((h>>>12)&2),1,1);
	drawBlockBevel(g,px,py,'rgba(255,255,255,0.40)','rgba(21,87,130,0.36)');
	g.restore();
}
function drawMotherLavaArt(g,px,py,h){
	g.save();
	// a solid relic: black volcanic plates split by a living magma heart
	g.fillStyle='rgba(29,8,4,0.58)';
	g.beginPath(); g.moveTo(px,py+1); g.lineTo(px+8,py+1); g.lineTo(px+6,py+7); g.lineTo(px+1,py+10); g.closePath(); g.fill();
	g.beginPath(); g.moveTo(px+13,py); g.lineTo(px+20,py); g.lineTo(px+20,py+9); g.lineTo(px+16,py+7); g.closePath(); g.fill();
	g.beginPath(); g.moveTo(px,py+14); g.lineTo(px+7,py+11); g.lineTo(px+10,py+20); g.lineTo(px,py+20); g.closePath(); g.fill();
	g.beginPath(); g.moveTo(px+13,py+12); g.lineTo(px+20,py+10); g.lineTo(px+20,py+20); g.lineTo(px+11,py+20); g.closePath(); g.fill();
	g.strokeStyle='rgba(255,245,150,0.92)'; g.lineWidth=1.4; g.lineJoin='round';
	g.beginPath();
	g.moveTo(px+9,py); g.lineTo(px+8,py+6); g.lineTo(px+11,py+9); g.lineTo(px+8,py+13); g.lineTo(px+10,py+20);
	g.moveTo(px,py+12); g.lineTo(px+6,py+10); g.lineTo(px+11,py+9); g.lineTo(px+16,py+10); g.lineTo(px+20,py+8);
	g.stroke();
	g.strokeStyle='rgba(255,81,16,0.78)'; g.lineWidth=3;
	g.beginPath(); g.moveTo(px+9,py+1); g.lineTo(px+9,py+6); g.lineTo(px+11,py+9); g.lineTo(px+9,py+13); g.lineTo(px+10,py+19); g.stroke();
	const ox=((h>>>6)&1), oy=((h>>>9)&1);
	g.fillStyle='rgba(255,74,13,0.95)';
	g.beginPath(); g.moveTo(px+10+ox,py+5+oy); g.lineTo(px+15+ox,py+10+oy); g.lineTo(px+10+ox,py+15+oy); g.lineTo(px+6+ox,py+10+oy); g.closePath(); g.fill();
	g.fillStyle='rgba(255,235,112,0.94)'; g.fillRect(px+9+ox,py+8+oy,3,4);
	g.fillStyle='rgba(255,255,218,0.94)'; g.fillRect(px+10+ox,py+8+oy,1,2);
	g.fillStyle='rgba(136,44,18,0.45)'; g.fillRect(px+2,py+3,4,2); g.fillRect(px+14,py+15,4,2);
	drawBlockBevel(g,px,py,'rgba(255,189,58,0.35)','rgba(41,5,2,0.55)');
	g.restore();
}
function drawGraphiteArt(g,px,py,h){
	g.save();
	// flaky graphite cleaves into stacked, offset crystal planes
	g.fillStyle='rgba(8,10,14,0.34)'; g.fillRect(px+1,py+1,TILE-2,TILE-2);
	for(let i=0;i<5;i++){
		const yy=py+3+i*3+((h>>>(i+4))&1);
		const off=(i&1)?3:1;
		g.fillStyle=i&1?'rgba(20,22,28,0.62)':'rgba(89,94,105,0.34)';
		g.beginPath(); g.moveTo(px+off,yy); g.lineTo(px+15+((h>>>(i+9))&3),yy-1); g.lineTo(px+TILE-off,yy+1); g.lineTo(px+4,yy+3); g.closePath(); g.fill();
		g.strokeStyle=i===1?'rgba(214,222,231,0.36)':'rgba(132,142,154,0.22)'; g.lineWidth=1;
		g.beginPath(); g.moveTo(px+off+1,yy); g.lineTo(px+TILE-off-2,yy+1); g.stroke();
	}
	g.fillStyle='rgba(235,242,248,0.54)'; g.fillRect(px+4+((h>>>14)&7),py+4+((h>>>18)&7),4,1);
	g.fillStyle='rgba(0,0,0,0.42)'; g.fillRect(px+2,py+TILE-3,TILE-4,2);
	drawBlockBevel(g,px,py,'rgba(201,210,220,0.16)','rgba(0,0,0,0.48)');
	g.restore();
}
function drawGrapheneArt(g,px,py,h){
	g.save();
	// engineered carbon sheet: layered edges plus a crisp hexagonal lattice
	g.fillStyle='rgba(8,13,19,0.62)'; g.fillRect(px+1,py+1,TILE-2,TILE-2);
	g.fillStyle='rgba(117,137,154,0.24)'; g.fillRect(px+2,py+3,TILE-4,2); g.fillRect(px+4,py+16,TILE-6,1);
	g.strokeStyle='rgba(124,181,205,0.68)'; g.lineWidth=1;
	const hex=(cx0,cy0)=>{
		g.beginPath(); g.moveTo(cx0-3,cy0); g.lineTo(cx0-1.5,cy0-3); g.lineTo(cx0+1.5,cy0-3); g.lineTo(cx0+3,cy0); g.lineTo(cx0+1.5,cy0+3); g.lineTo(cx0-1.5,cy0+3); g.closePath(); g.stroke();
	};
	const shift=(h>>>8)&1;
	hex(px+5+shift,py+7); hex(px+11+shift,py+7); hex(px+8+shift,py+12); hex(px+14+shift,py+12);
	g.fillStyle='rgba(203,239,250,0.82)';
	for(const p of [[5,4],[8,9],[11,4],[14,9],[8,15]]) g.fillRect(px+p[0]+shift,py+p[1],1,1);
	g.fillStyle='rgba(87,235,255,0.70)'; g.fillRect(px+3+((h>>>12)&9),py+3,4,1);
	g.strokeStyle='rgba(220,245,255,0.26)'; g.strokeRect(px+1.5,py+1.5,TILE-3,TILE-3);
	drawBlockBevel(g,px,py,'rgba(226,248,255,0.34)','rgba(3,8,14,0.54)');
	g.restore();
}
function drawSmrCellArt(g,px,py,h){
	g.save();
	// compact reactor cassette: armored shell, hazard bands and a hot core window
	drawBlockBevel(g,px,py,'rgba(190,230,209,0.27)','rgba(5,14,12,0.52)');
	g.fillStyle='rgba(12,27,24,0.78)'; g.fillRect(px+2,py+2,TILE-4,TILE-4);
	g.fillStyle='rgba(78,111,91,0.72)'; g.fillRect(px+3,py+3,TILE-6,3); g.fillRect(px+3,py+14,TILE-6,3);
	g.fillStyle='rgba(229,190,55,0.84)';
	for(let i=0;i<4;i++){
		const x=px+3+i*4;
		g.beginPath(); g.moveTo(x,py+4); g.lineTo(x+2,py+2); g.lineTo(x+4,py+2); g.lineTo(x+2,py+5); g.closePath(); g.fill();
	}
	g.fillStyle='rgba(2,8,9,0.92)'; g.fillRect(px+5,py+6,10,8);
	g.strokeStyle='rgba(107,255,188,0.68)'; g.lineWidth=1; g.strokeRect(px+5.5,py+6.5,9,7);
	const ox=((h>>>9)&1);
	g.fillStyle='rgba(195,255,119,0.88)';
	g.beginPath(); g.arc(px+10+ox,py+10,3.1,0,Math.PI*2); g.fill();
	g.fillStyle='rgba(16,40,28,0.88)';
	for(let i=0;i<3;i++){
		const a=i*Math.PI*2/3-0.35;
		g.beginPath(); g.moveTo(px+10+ox,py+10); g.arc(px+10+ox,py+10,2.6,a,a+0.72); g.closePath(); g.fill();
	}
	g.fillStyle='rgba(232,255,196,0.98)'; g.fillRect(px+9+ox,py+9,2,2);
	g.fillStyle='rgba(126,255,211,0.78)'; g.fillRect(px+4,py+16,3,1);
	g.fillStyle='rgba(255,214,70,0.74)'; g.fillRect(px+13,py+16,3,1);
	g.restore();
}
function drawAltarTileArt(g,px,py,h){
	g.save();
	g.fillStyle='rgba(24,7,37,0.56)'; g.fillRect(px+2,py+3,TILE-4,TILE-5);
	g.fillStyle='rgba(70,28,91,0.82)'; g.fillRect(px+1,py+15,TILE-2,4); g.fillRect(px+3,py+12,TILE-6,4);
	g.strokeStyle='rgba(235,145,255,0.80)'; g.lineWidth=1.2;
	g.beginPath(); g.moveTo(px+4,py+12); g.lineTo(px+10,py+3); g.lineTo(px+16,py+12); g.lineTo(px+10,py+9); g.closePath(); g.stroke();
	g.fillStyle='rgba(255,211,105,0.92)'; g.fillRect(px+9,py+7,2,2);
	g.fillStyle='rgba(205,106,255,0.56)'; g.fillRect(px+5+((h>>>8)&6),py+4,2,1);
	drawBlockBevel(g,px,py,'rgba(241,185,255,0.22)','rgba(16,3,25,0.46)');
	g.restore();
}
function drawGlowshroomTileArt(g,px,py,h){
	g.save();
	g.fillStyle='rgba(18,8,25,0.38)'; g.fillRect(px+6,py+17,9,2);
	g.fillStyle='#6b806f'; g.fillRect(px+9,py+9,3,8); g.fillRect(px+8,py+15,5,2);
	g.fillStyle='#38a982';
	g.beginPath(); g.moveTo(px+3,py+10); g.quadraticCurveTo(px+5,py+3,px+10,py+3); g.quadraticCurveTo(px+16,py+3,px+18,py+10); g.closePath(); g.fill();
	g.fillStyle='rgba(145,255,211,0.82)'; g.fillRect(px+5,py+7,11,2); g.fillRect(px+7,py+5,6,1);
	g.fillStyle='rgba(226,255,238,0.94)'; g.fillRect(px+7+((h>>>6)&5),py+5,2,2); g.fillRect(px+13,py+8,1,1);
...
function drawLeafPileTileArt(g,px,py,h){
	...
	const colors=['#7f3d20','#ad6427','#d68b2e','#efb94c','#8c7027'];
	for(let i=0;i<13;i++){
		const r=hash32(h+i*97,i*311);
		const x=px+2+((r>>>4)%16), yy=py+10+((r>>>9)%8);
		g.fillStyle=colors[(r>>>14)%colors.length];
		g.fillRect(x,yy,2+((r>>>18)&1),1+((r>>>20)&1));
		if((r&3)===0){ g.fillStyle='rgba(255,218,114,0.62)'; g.fillRect(x,yy,1,1); }
	}
	g.fillStyle='rgba(255,202,78,0.40)'; g.fillRect(px+5+((h>>>7)&5),py+11,5,1);
	g.restore();
}
function drawMeteorDustTileArt(g,px,py,wx,wy,h,conn){
	g.save();
	conn=conn||{};
	wx=Number.isFinite(wx)?wx:0; wy=Number.isFinite(wy)?wy:0;
	// Ethereal sky matter: neighboring cells share row/column edge anchors, so
	// floating deposits merge into one branching nebula instead of tiled piles.
	const cx0=px+8+((h>>>5)&4), cy0=py+8+((h>>>9)&4);
	const rowY=py+6+(hash32(173,wy)%9);
	const colX=px+6+(hash32(wx,911)%9);
	g.lineCap='round'; g.lineJoin='round';
	const tendrils=()=>{
		g.beginPath();
		if(conn.left){ g.moveTo(cx0,cy0); g.quadraticCurveTo(px+5,cy0,px,rowY); }
		if(conn.right){ g.moveTo(cx0,cy0); g.quadraticCurveTo(px+15,cy0,px+TILE,rowY); }
		if(conn.up){ g.moveTo(cx0,cy0); g.quadraticCurveTo(cx0,py+5,colX,py); }
		if(conn.down){ g.moveTo(cx0,cy0); g.quadraticCurveTo(cx0,py+15,colX,py+TILE); }
		g.stroke();
	};
	g.strokeStyle='rgba(95,63,177,0.24)'; g.lineWidth=7; tendrils();
	g.strokeStyle='rgba(174,125,239,0.34)'; g.lineWidth=4; tendrils();
	g.strokeStyle='rgba(115,225,255,0.46)'; g.lineWidth=1.2; tendrils();
	// Soft overlapping vapor lobes keep isolated cells readable while leaving
	// plenty of sky visible through a large suspended field.
	g.globalCompositeOperation='lighter';
	g.fillStyle='rgba(96,58,168,0.22)';
	g.beginPath(); g.ellipse(cx0,cy0,7,5.2,-0.28,0,Math.PI*2); g.fill();
	g.fillStyle='rgba(188,118,240,0.20)';
	g.beginPath(); g.ellipse(cx0-3,cy0+2,5.4,3.5,0.42,0,Math.PI*2); g.fill();
	g.fillStyle='rgba(83,209,243,0.16)';
	g.beginPath(); g.ellipse(cx0+3,cy0-2,4.8,3.2,-0.56,0,Math.PI*2); g.fill();
	g.strokeStyle='rgba(215,174,255,0.58)'; g.lineWidth=1;
	g.beginPath(); g.ellipse(cx0,cy0,6.2,3.4,0.55,0,Math.PI*2); g.stroke();
	// A tiny prismatic nucleus and sparse stars break up repeated large fields.
	const flip=(h>>>14)&1;
	g.fillStyle='rgba(205,143,255,0.84)';
	g.beginPath(); g.moveTo(cx0,cy0-3); g.lineTo(cx0+3,cy0); g.lineTo(cx0,cy0+4); g.lineTo(cx0-2,cy0); g.closePath(); g.fill();
	g.fillStyle='rgba(135,235,255,0.88)';
	g.beginPath(); g.moveTo(cx0,cy0-3); g.lineTo(cx0+3,cy0); g.lineTo(cx0,cy0); g.closePath(); g.fill();
	g.fillStyle='rgba(255,255,255,0.94)'; g.fillRect(cx0,cy0-1,1,2);
	g.fillStyle='rgba(211,169,255,0.82)'; g.fillRect(px+3+flip*11,py+4+((h>>>17)&5),1,1);
	g.fillStyle='rgba(118,231,255,0.78)'; g.fillRect(px+15-flip*10,py+13-((h>>>20)&4),1,1);
	g.restore();
}
function drawAntimatterCrystalArt(g,px,py,h){
	g.save();
	// A dark containment field frames several sharp crystal shards. The cyan
	// interference edge separates antimatter from ordinary purple materials.
	g.fillStyle='rgba(5,2,15,0.82)'; g.fillRect(px+1,py+1,TILE-2,TILE-2);
	g.fillStyle='rgba(55,17,81,0.78)'; g.fillRect(px+3,py+3,TILE-6,TILE-6);
	const shard=(x0,y0,w0,h0,fill,edge)=>{
		g.fillStyle=fill;
		g.beginPath(); g.moveTo(x0,y0); g.lineTo(x0+w0,y0+h0*0.42); g.lineTo(x0+w0*0.58,y0+h0); g.lineTo(x0-w0*0.24,y0+h0*0.58); g.closePath(); g.fill();
		g.strokeStyle=edge; g.lineWidth=1; g.stroke();
	};
	const shift=(h>>>7)&1;
	shard(px+9+shift,py+2,5,15,'rgba(210,91,255,0.92)','rgba(122,239,255,0.92)');
	shard(px+4,py+7,4,10,'rgba(139,55,211,0.88)','rgba(235,181,255,0.72)');
	shard(px+14,py+9,3,8,'rgba(103,37,171,0.88)','rgba(107,222,255,0.72)');
	g.fillStyle='rgba(14,3,30,0.94)';
	g.beginPath(); g.moveTo(px+10+shift,py+6); g.lineTo(px+13+shift,py+10); g.lineTo(px+10+shift,py+14); g.lineTo(px+8+shift,py+10); g.closePath(); g.fill();
	g.fillStyle='rgba(255,245,255,0.96)'; g.fillRect(px+10+shift,py+4,1,3); g.fillRect(px+6,py+9,1,2);
	g.strokeStyle='rgba(102,231,255,0.42)'; g.lineWidth=1;
	g.beginPath(); g.moveTo(px+2,py+4); g.lineTo(px+5,py+2); g.moveTo(px+15,py+4); g.lineTo(px+18,py+7); g.moveTo(px+3,py+17); g.lineTo(px+7,py+18); g.stroke();
	drawBlockBevel(g,px,py,'rgba(231,187,255,0.38)','rgba(0,0,8,0.66)');
	g.restore();
}
function drawTorchPreviewArt(g,px,py,h){
	g.save();
	g.fillStyle='rgba(22,10,3,0.32)'; g.fillRect(px+7,py+17,7,2);
	g.fillStyle='#6e3d1f'; g.fillRect(px+9,py+8,3,9);
	g.fillStyle='#d09a50'; g.fillRect(px+10,py+8,1,8);
	g.fillStyle='rgba(255,95,22,0.92)';
	g.beginPath(); g.moveTo(px+10,py+10); g.quadraticCurveTo(px+5,py+6,px+9,py+1); g.quadraticCurveTo(px+15,py+5,px+12,py+10); g.closePath(); g.fill();
	g.fillStyle='rgba(255,228,103,0.96)';
	g.beginPath(); g.moveTo(px+10,py+8); g.quadraticCurveTo(px+8,py+5,px+10+((h>>>7)&1),py+3); g.quadraticCurveTo(px+13,py+6,px+11,py+9); g.closePath(); g.fill();
	g.restore();
}
function drawWaterPreviewArt(g,px,py,h){
	g.fillStyle='rgba(8,38,112,0.24)'; g.fillRect(px,py+13,TILE,7);
	g.strokeStyle='rgba(178,231,255,0.74)'; g.lineWidth=1.2;
	g.beginPath();
	const wobble=(h>>>6)&1;
	g.moveTo(px,py+5+wobble); g.quadraticCurveTo(px+5,py+2,px+10,py+5+wobble); g.quadraticCurveTo(px+15,py+8,px+20,py+4+wobble); g.stroke();
	g.strokeStyle='rgba(86,177,255,0.58)';
	g.beginPath(); g.moveTo(px+2,py+11); g.quadraticCurveTo(px+7,py+8,px+12,py+11); g.quadraticCurveTo(px+16,py+13,px+19,py+10); g.stroke();
	g.fillStyle='rgba(222,248,255,0.72)'; g.fillRect(px+5+((h>>>9)&6),py+14,1,1); g.fillRect(px+15,py+8,1,1);
}
function drawMaterialIdentityArt(g,t,px,py,h){
	if(t===T.MOTHER_ICE) drawMotherIceArt(g,px,py,h);
	else if(t===T.MOTHER_LAVA) drawMotherLavaArt(g,px,py,h);
	else if(t===T.GRAPHITE) drawGraphiteArt(g,px,py,h);
	else if(t===T.GRAPHENE) drawGrapheneArt(g,px,py,h);
	else if(t===T.SMR_CELL) drawSmrCellArt(g,px,py,h);
	else if(t===T.ALTAR) drawAltarTileArt(g,px,py,h);
	else if(t===T.ANTIMATTER_CRYSTAL) drawAntimatterCrystalArt(g,px,py,h);
	else if(t===T.VOLCANO_MASTER_STONE || t===T.SERVANT_STONE) _drawMaterialTile(g,t,px,py,h);
}
...
// Weather-instrument hotbar/preview art: an OBJECT silhouette (mast + head),
// never a filled block — so the icon reads like the rooftop fixture it places.
// The live in-world arrow/charge overlay is drawn by WEATHER_INSTRUMENTS.
function drawWeatherInstrumentPreview(g,px,py,t){
	const cx=px+TILE*0.5, base=py+TILE-3;
	if(t===T.WEATHERVANE){
		g.strokeStyle='#7c8894';
		g.lineWidth=Math.max(1.4,TILE*0.09);
		g.beginPath(); g.moveTo(cx,base); g.lineTo(cx,py+4); g.stroke();
		// N/S cross + an arrow riding the top
		g.strokeStyle='#5f6a74';
		g.lineWidth=Math.max(1,TILE*0.05);
		g.beginPath(); g.moveTo(cx-4,py+9); g.lineTo(cx+4,py+9); g.stroke();
		g.fillStyle='#e0b45c';
		g.beginPath();
		g.moveTo(px+4,py+6); g.lineTo(cx+2,py+3); g.lineTo(cx+2,py+9); g.closePath(); g.fill();
		g.fillStyle='rgba(224,180,92,0.7)';
		g.fillRect(cx+2,py+4.5,TILE*0.28,2);
	} else {
		// lightning rod: a metal mast with a bright tip and a spark
		g.strokeStyle='#9fb2c4';
		g.lineWidth=Math.max(1.5,TILE*0.1);
		g.beginPath(); g.moveTo(cx,base); g.lineTo(cx,py+4); g.stroke();
		g.fillStyle='#cfe0ee';
		g.beginPath(); g.arc(cx,py+4,Math.max(1.5,TILE*0.1),0,Math.PI*2); g.fill();
		g.strokeStyle='rgba(255,244,180,0.9)';
		g.lineWidth=Math.max(1,TILE*0.05);
		g.beginPath(); g.moveTo(cx+1,py+6); g.lineTo(cx+5,py+11); g.lineTo(cx-1,py+14); g.stroke();
	}
}
...
function hasTerrainPattern(t){
	return t===T.SAND || t===T.UNSTABLE_SAND || t===T.QUICKSAND || t===T.CLAY || t===T.WET_CLAY || t===T.BRICK || t===T.CHIMNEY || t===T.DIRT || t===T.STONE || t===T.GRANITE || t===T.BASALT || t===T.BEDROCK || t===T.COAL || t===T.SILVER_INGOT || t===T.UFO_CONCRETE || t===T.FROZEN_DIRT || t===T.FROZEN_SAND || t===T.FROZEN_CLAY;
}
...
function terrainPatternCanvas(t,variant){
	...
	if(t===T.SILVER_INGOT){
		// Six tiny transparent sprites amortize the richer metal work to one
		// drawImage per tile whenever a chunk needs rebuilding.
		drawSilverIngotArt(g,0,0,hash32(t*131+variant*17,variant*241+7));
	}
	else if(t===T.SAND || t===T.UNSTABLE_SAND || t===T.QUICKSAND){
...
function _drawMaterialTile(g,t,px,py,h){
	...
	} else if(t===T.SILVER_ORE){
		drawBlockBevel(g,px,py,'rgba(236,247,255,.22)','rgba(25,31,40,.30)');
		dot(g,px,py,2,2,TILE-4,TILE-4,'rgba(75,80,88,.30)');
		drawSilverOreArt(g,px,py,h);
	} else if(t===T.SILVER_INGOT){
		drawSilverIngotArt(g,px,py,h);
	} else if(t===T.DIAMOND){
...
function drawEntityTileDetails(g,t,px,py,h,opts){
	...
	if(isPowerCableTileId(t)){
		const conn={left:true,right:true,up:false,down:false};
		if(TELEPORTERS && TELEPORTERS.drawCableTile) TELEPORTERS.drawCableTile(g,TILE,px,py,conn,h,t);
	}
	if(t===T.ELECTRONICS){
...
function drawEntityTile(g,t,px,py,wx,wy,opts){
	...
	if(isFurnishingTileId(t)){
		// Home furnishings are procedural open fixtures; the backdrop remains
		// visible around their silhouettes instead of becoming a solid square.
		FURNISHINGS.drawTile(g,t,px,py,wx,wy,opts||null);
		g.restore();
		return true;
	}
	// Passable fixtures keep transparent corners in previews. Filling these from
	// INFO.color was the most obvious source of placeholder-looking hotbar icons.
	if(t===T.TORCH){
		drawTorchPreviewArt(g,px,py,h);
		g.restore();
		return true;
	}
	if(t===T.RESPAWN_TOTEM){
		drawRespawnTotemTile(g,px,py);
		g.restore();
		return true;
	}
	if(t===T.WEATHERVANE || t===T.LIGHTNING_ROD){
		drawWeatherInstrumentPreview(g,px,py,t);
		g.restore();
		return true;
	}
	if(t===T.GLOWSHROOM){
		drawGlowshroomTileArt(g,px,py,h);
		g.restore();
		return true;
	}
	if(t===T.LEAF_PILE){
	...
	if(t===T.METEOR_DUST){
		drawMeteorDustTileArt(g,px,py,wx,wy,h,{left:true,right:true,up:true,down:true});
		g.restore();
		return true;
	}
	if(t===T.MEAT || t===T.ROTTEN_MEAT || t===T.BAKED_MEAT){
		drawMeatTile(g,px,py,t===T.ROTTEN_MEAT?'rotten':(t===T.BAKED_MEAT?'baked':'fresh'),h);
		g.restore();
		return true;
	}
	if(isLadderTileId(t)){
		drawLadderOverlay(g,px,py,h,{up:true,down:true},t);
		g.restore();
		return true;
	}
	if(t===T.WIRE || isPowerCableTileId(t) || t===T.WATER_PIPE){
		if(t===T.WATER_PIPE) _drawMaterialTile(g,t,px,py,h);
		else drawEntityTileDetails(g,t,px,py,h,opts||null);
		g.restore();
		return true;
	}
	if(t===T.GOLD_ORE || t===T.SILVER_ORE){
		...
		if(t===T.GOLD_ORE) drawGoldOreArt(g,px,py,h); else drawSilverOreArt(g,px,py,h);
	}else if(t===T.DIAMOND){
	...
	// The broad material renderer already contains bespoke rock, metal, machine
	// and relic silhouettes. Reuse it in UI previews for types that previously
	// stopped after the base fill, while avoiding the ore/detail passes above.
	if(t!==T.GOLD_ORE && t!==T.SILVER_ORE && t!==T.DIAMOND &&
		t!==T.GLASS && !isDoorTile(t) && !isTrapdoorTile(t) &&
		t!==T.ELECTRONICS && t!==T.TRANSISTOR && t!==T.SOLAR_PANEL && t!==T.SOLAR_BATTERY &&
		t!==T.SPRING_PLATFORM && t!==T.DYNAMO && t!==T.DYNAMO_SLOT && t!==T.COAL && t!==T.STEEL && t!==T.TRACK && t!==T.ANTIMATTER_CRYSTAL){
		_drawMaterialTile(g,t,px,py,h);
	}
	if(isLeaf(t)) drawLeafClumpArt(g,t,px,py,wx,wy,h);
	if(t===T.MUD) drawMudDetail(g,px,py,h);
	if(t===T.LAVA) drawLavaCrustArt(g,px,py,h);
	if(t===T.WATER) drawWaterPreviewArt(g,px,py,h);
	drawMaterialIdentityArt(g,t,px,py,h);
	if(INFO[t] && INFO[t].chestTier) drawChestTile(g,px,py,t,h);
	if(t===T.INVASION_CACHE) drawInvasionCacheTile(g,px,py,h);
	if(t===T.VENDING_MACHINE) drawVendingMachineTile(g,px,py,h,0);
	...
	cctx.translate(0,-originY*TILE);
		for(let lx=0; lx<CHUNK_W; lx++){
			...
			for(let y=redrawWorldY0;y<=redrawWorldY1;y++){
				...
				if(hasBg){
					if((t===T.AIR || t===T.WATER || t===T.TORCH || t===T.GRAVE || t===T.RESPAWN_TOTEM || openArtTile || isFurnishingTileId(t) || t===T.WIRE || isPowerCableTileId(t) || t===T.WATER_PIPE || gasTile) && y>surf && !(gasTile && gasSkyExposedTile(wx,y))){
						drawUndergroundBackdrop(cctx,lx*TILE,y*TILE,wx,y,surf);
					}
					drawBackgroundBuildTile(cctx,bgT,lx*TILE,y*TILE,wx,y,hash32(wx,y));
				}
				if(t===T.AIR || t===T.WATER || t===T.TORCH || t===T.GRAVE || t===T.RESPAWN_TOTEM || openArtTile || isFurnishingTileId(t) || t===T.WIRE || isPowerCableTileId(t) || t===T.WATER_PIPE || gasTile){
					...
					if(t===T.METEOR_DUST){
						const dustConn={
							left:chunkTileAt(arr,cx,lx-1,y,originY,sectionH)===T.METEOR_DUST,
							right:chunkTileAt(arr,cx,lx+1,y,originY,sectionH)===T.METEOR_DUST,
							up:chunkTileAt(arr,cx,lx,y-1,originY,sectionH)===T.METEOR_DUST,
							down:chunkTileAt(arr,cx,lx,y+1,originY,sectionH)===T.METEOR_DUST
						};
						drawMeteorDustTileArt(cctx,lx*TILE,y*TILE,wx,y,hash32(wx,y),dustConn);
					}
					if(isChairTileId(t)) drawChairTile(cctx, lx*TILE, y*TILE, t);
					else if(isFurnishingTileId(t)) FURNISHINGS.drawTile(cctx,t,lx*TILE,y*TILE,wx,y,null);
					if(t===T.WIRE){
					...
					if(isPowerCableTileId(t)){
						...
						if(TELEPORTERS && TELEPORTERS.drawCableTile) TELEPORTERS.drawCableTile(cctx,TILE,px,py,conn,h,t);
					}
					continue;
				}
				const h = hash32(wx,y);
				...
				if(t===T.GOLD_ORE || t===T.SILVER_ORE){
					...
					if(t===T.GOLD_ORE) drawGoldOreArt(cctx,lx*TILE,y*TILE,h); else drawSilverOreArt(cctx,lx*TILE,y*TILE,h);
				}else if(t===T.DIAMOND){
				...
				if(t===T.MOTHER_ICE || t===T.MOTHER_LAVA || t===T.GRAPHITE || t===T.GRAPHENE || t===T.SMR_CELL || t===T.ALTAR || t===T.ANTIMATTER_CRYSTAL || t===T.VOLCANO_MASTER_STONE || t===T.SERVANT_STONE){
					drawMaterialIdentityArt(cctx,t,lx*TILE,y*TILE,h);
				}
...
function drawLadderOverlay(g,px,py,h,conn,kind){
	g.save();
	conn=conn||{};
	const bedrock=kind===T.BEDROCK_LADDER;
	...
	const railShadow=bedrock?'rgba(22,27,38,0.82)':'rgba(58,34,17,0.72)';
	const rail=bedrock?'rgba(103,115,140,0.98)':'rgba(184,124,61,0.94)';
	const railHi=bedrock?'rgba(188,205,232,0.62)':'rgba(244,185,96,0.56)';
	const rung=bedrock?'rgba(133,148,177,0.98)':'rgba(203,145,75,0.95)';
	const rungDark=bedrock?'rgba(39,47,64,0.86)':'rgba(88,48,22,0.72)';
	...
	if(!up || !down){
		g.fillStyle=bedrock?'rgba(35,42,57,0.78)':'rgba(74,40,18,0.62)';
	...
	for(let y=4;y<TILE-2;y+=5){
		...
		g.fillStyle=bedrock?'rgba(221,232,255,0.40)':'rgba(255,221,143,0.36)';
		g.fillRect(px+6,py+y,8,1);
	}
	if(((h>>>6)&3)===0){
		g.fillStyle=bedrock?'rgba(226,238,255,0.52)':'rgba(255,231,160,0.45)';
...
function drawInfrastructureOverlays(sx,sy,viewX,viewY,opts){
	if(!WORLD || !WORLD.getInfrastructure) return;
	opts=opts||{};
	const only=opts.only==null?null:(Array.isArray(opts.only)?opts.only:[opts.only]);
	const exclude=opts.exclude==null?null:(Array.isArray(opts.exclude)?opts.exclude:[opts.exclude]);
	...
	for(const cell of cells){
		for(const t of cell.tiles){
			...
			if(t===T.WIRE) drawLooseWireOverlay(ctx,cell.px,cell.py,cell.h);
				...
				const mode=PUMPS.pipeModeAt ? PUMPS.pipeModeAt(cell.x,cell.y,getFluidNetworkTile) : 'normal';
				PUMPS.drawPipeTile(ctx,TILE,cell.px,cell.py,conn,cell.h,mode);
			}else if(isLadderTileId(t)){
				drawLadderOverlay(ctx,cell.px,cell.py,cell.h,ladderConnections(cell.x,cell.y,hasLadderAt),t);
			}else if(isPowerCableTileId(t) && TELEPORTERS && TELEPORTERS.drawCableTile){
				...
				TELEPORTERS.drawCableTile(ctx,TILE,cell.px,cell.py,conn,cell.h,t);
			}
		}
	}
}
...
let smokeDoorActors=[];
let smokeDoorActorsAt=-Infinity;
function refreshSmokeDoorActors(){
	const now=performance.now();
	if(now-smokeDoorActorsAt<160) return;
	smokeDoorActorsAt=now;
	try{ smokeDoorActors=(NPCS && NPCS.summaries) ? NPCS.summaries() : []; }
	catch(e){ smokeDoorActors=[]; }
}
function smokeDynamicOpenAt(x,y,t){
	if(isDoorTile(t)) return actorOpensDoor(x,y,smokeDoorActors);
	if(isTrapdoorTile(t)) return actorOpensTrapdoor(x,y,smokeDoorActors);
	return false;
}
...
function currentDaylight(){
	try{
		...
		if(ti){
			...
			const night=Number.isFinite(Number(ti.moonlight))
				? Math.max(0,Math.min(0.22,Number(ti.moonlight)))
				: 0.018*Math.sin(tDay*Math.PI);
			if(!ti.isDay) return night;
			// Day follows the solar arc — but at the boundaries the arc used to hit
			// exactly 0 while the night side sat at the moonlight level, so sunset
			// SNAPPED brighter and dawn started pitch black under a lit sky. The
			// twilight shoulder now eases the arc into the night level instead.
			const arc=Math.max(0,Math.sin(tDay*Math.PI));
			const band=Math.max(0.02,Number(ti.twilightBand)||0.12);
			const edge=Math.min(1,Math.min(tDay,1-tDay)/band);
			const k=edge*edge*(3-2*edge); // smoothstep into full daylight
			return night+(Math.max(arc,night)-night)*k;
		}
	}catch(e){}
	return 1;
}
...
function collectThermalVisionTargets(){
	const out=[];
	const push=(entry)=>{
		if(!entry || out.length>=320 || !Number.isFinite(Number(entry.x)) || !Number.isFinite(Number(entry.y))) return;
		out.push(entry);
	};
	const heroH=Math.max(0.2,Number(player.h)||0.95);
	push({id:'hero',x:player.x,y:player.y-heroH*0.5,w:player.w,h:heroH,hp:player.hp,heat:0.92,living:true,center:true});
	try{
		if(MOBS && MOBS.thermalTargets){
			const poses=MOBS.thermalTargets(player.x,player.y,28,80)||[];
			for(let i=0;i<poses.length;i++){
				const p=poses[i],h=Math.max(0.2,Number(p.h)||1);
				push({...p,id:'mob_'+i+'_'+String(p.id||''),y:Number(p.y)-h*0.5,h,center:true});
			}
		}
	}catch(e){}
	try{
		const nearby=NPCS && NPCS.nearby ? NPCS.nearby(player,28) : [];
		for(const n of nearby.slice(0,24)){
			const h=Math.max(0.65,Number(n.h)||0.95);
			push({id:'npc_'+String(n.id||out.length),name:n.name||'NPC',x:n.x,y:Number(n.y)-h*0.5,w:Number(n.w)||0.7,h,hp:Number.isFinite(Number(n.hp))?Number(n.hp):1,heat:0.74,living:true,center:true});
		}
	}catch(e){}
	try{
		const units=INVASIONS && INVASIONS.thermalTargets ? (INVASIONS.thermalTargets(player.x,player.y,28,24)||[]) : [];
		for(let i=0;i<units.length;i++){
			const u=units[i],h=Math.max(0.3,Number(u.h)||1);
			push({...u,id:'invasion_'+i+'_'+String(u.id||''),y:Number(u.y)-h*0.5,h,center:true});
		}
	}catch(e){}
	try{
		if(BOSSES && BOSSES.targetsForTurret){
			const rows=BOSSES.targetsForTurret(player.x,player.y,26)||[];
			const seen=new Set();
			for(const row of rows.slice(0,48)){
				const boss=row && row.boss;
				if(!boss || seen.has(boss)) continue;
				seen.add(boss);
				const w=Math.max(1,Number(boss.maxDx)-Number(boss.minDx)+1||2);
				const h=Math.max(1,Number(boss.height)||2);
				push({id:'boss_'+seen.size,x:Number(boss.x)+(Number(boss.minDx)+Number(boss.maxDx)+1)*0.5,y:Number(boss.y)-h*0.5,w,h,hp:row.hp,heat:1,living:true,center:true});
			}
		}
	}catch(e){}
	for(const [api,prefix,heat] of [[GUARDIANS,'guardian',0.94],[UNDERGROUND,'underground',0.96],[SKY_GUARDIAN,'sky',0.9]]){
		try{
			const rows=api && api.targetsForTurret ? (api.targetsForTurret(player.x,player.y,28,false)||[]) : [];
			for(const row of rows.slice(0,32)){
				const e=row.raw||row.guardian||row.underground||row.skyGuardian||{};
				const h=Math.max(0.6,Number(e.h)||Number(e.height)||1.2);
				const w=Math.max(0.5,Number(e.w)||Number(e.width)||0.9);
				push({id:prefix+'_'+String(e.id||out.length),x:Number(row.x),y:Number(row.y)-h*0.5,w,h,hp:row.hp,heat,living:true,center:true});
			}
		}catch(e){}
	}
	try{
		const list=COMPANIONS && COMPANIONS._debug && COMPANIONS._debug.list ? COMPANIONS._debug.list() : [];
		for(const c of list.slice(0,48)) push({id:'companion_'+c.id,x:c.x,y:Number(c.y)-0.55,w:0.9,h:1.1,hp:c.hp,heat:c.kind==='water'?0.34:0.7,living:true,center:true});
	}catch(e){}
	const priority=entry=>{
		const id=String(entry&&entry.id||'');
		if(id==='hero' || /^(boss|guardian|underground|sky)_/.test(id)) return 3;
		if(/^(invasion|companion)_/.test(id)) return 2;
		if(id.startsWith('npc_')) return 1;
		return 0;
	};
	out.sort((a,b)=>priority(b)-priority(a) || Math.hypot(Number(a.x)-player.x,Number(a.y)-player.y)-Math.hypot(Number(b.x)-player.x,Number(b.y)-player.y));
	return out.slice(0,192);
}
function drawSpecialVisionOverlay(camRenderX,camRenderY,shake){
	if(!VISION_MODES || !VISION_MODES.isOn()) return false;
	const info=VISION_MODES.info();
	const targets=info.mode===VISION_MODE_IDS.THERMAL ? collectThermalVisionTargets() : [];
	ctx.save();
	try{
		ctx.setTransform(DPR,0,0,DPR,0,0);
		...
		const result=VISION_MODES.draw(ctx,{x:0,y:0,width:W,height:H,tileSize:TILE*zoom,worldX:camRenderX,worldY:camRenderY},{
			fogWillRenderAfter:true,
			targets,
			origin:{x:player.x,y:player.y},
			toScreen:(t,view)=>({
				x:(Number(t.x)-camRenderX)*view.tileSize+shakeX,
				y:(Number(t.y)-camRenderY)*view.tileSize+shakeY,
				w:Math.max(1,(Number(t.w)||0.8)*view.tileSize),
				h:Math.max(1,(Number(t.h)||1)*view.tileSize),
				center:true
			}),
			isVisible:t=>worldFxCurrentlyVisible(Math.floor(Number(t.x)),Math.floor(Number(t.y))),
			isLiving:t=>t && t.living!==false && !(Number.isFinite(Number(t.hp)) && Number(t.hp)<=0)
		});
		return !!(result && result.overlay && result.overlay.drawn);
	}catch(e){ return false; }
	finally{ ctx.restore(); }
}
...
function fullscreenActive(){ return !!fullscreenElement(); }
...
function syncFullscreenControls(){
	...
	const key=KEYBINDS ? KEYBINDS.displayKey(KEYBINDS.keyFor('fullscreen')) : 'U';
	try{
		const b=pausePanel && pausePanel.querySelector('.pauseFullscreenBtn');
		if(b){ b.textContent=fullscreenBtnLabel(); b.setAttribute('aria-pressed',String(active)); }
		const top=document.getElementById('fullscreenBtn');
		if(top){
			top.setAttribute('aria-pressed',String(active));
			top.classList.toggle('toggled',active);
			top.title=(active?'Wyłącz':'Włącz')+' pełny ekran ('+key+')';
			top.setAttribute('aria-label',(active?'Wyłącz':'Włącz')+' pełny ekran');
		}
	}catch(e){}
}
for(const eventName of ['fullscreenchange','webkitfullscreenchange']) document.addEventListener(eventName,syncFullscreenControls);
document.getElementById('fullscreenBtn')?.addEventListener('click',toggleFullscreen);
MM.fullscreen={active:fullscreenActive,supported:fullscreenSupported,toggle:toggleFullscreen,sync:syncFullscreenControls};
...
function ensurePausePanel(){
	...
	fs.setAttribute('aria-pressed',String(fullscreenActive()));
...
function radioPanelVisible(){ return !!(radioPanel && !radioPanel.hidden); }
function currentRadioInfo(){
	try{ if(AUDIO && AUDIO.getRadioStationInfo) return AUDIO.getRadioStationInfo(); }catch(e){}
	const fallback=RADIO_STATIONS.find(station=>station.id==='off')||RADIO_STATIONS[0];
	return Object.assign({},fallback,{track:null,active:false,blockedReason:'audio-suspended'});
}
function activateRadioAudio(){
	try{
		if(!AUDIO || typeof AUDIO.activate!=='function') return null;
		const pending=AUDIO.activate();
		if(pending && typeof pending.then==='function') pending.then(()=>{
			if(radioPanelVisible()) renderRadioPanel();
		}).catch(()=>{});
		return pending;
	}catch(e){ return null; }
}
function radioStatusText(info){
	if(!info || info.id==='off') return 'Odbiornik wyłączony';
	if(info.active) return 'NADAJE';
	const labels={
		'muted':'Dźwięk jest wyciszony w ustawieniach',
		'music-off':'Muzyka jest wyłączona w ustawieniach',
		'music-volume':'Głośność muzyki wynosi 0%',
		'no-source':'Podejdź do ustawionego radia',
		'no-power':'Brak zasilania: podłącz przewód, dynamo lub panel słoneczny',
		'danger':'Sygnał przerwany przez zagrożenie',
		'audio-suspended':'Kliknij stację, aby uruchomić dźwięk'
	};
	return labels[info.blockedReason]||'Strojenie sygnału…';
}
const HOME_FURNISHING_POWER={
	network:TELEPORTERS,
	dynamo:DYNAMO,
	solar:SOLAR,
	getNetworkTile:getElectricNetworkTile,
	onStateChanged:(x,y)=>{ if(LIGHTING && LIGHTING.onTileChanged) LIGHTING.onTileChanged(x,y); }
};
function furnishingPoweredAt(x,y){
	try{ return !!(FURNISHINGS && FURNISHINGS.isPoweredAt && FURNISHINGS.isPoweredAt(x,y,getTile,HOME_FURNISHING_POWER)); }
	catch(e){ return false; }
}
function publishRadioPower(){
	if(!radioTarget || !AUDIO || !AUDIO.setRadioSource) return false;
	const powered=furnishingPoweredAt(radioTarget.x,radioTarget.y);
	AUDIO.setRadioSource(radioTarget.x+.5,radioTarget.y+.5,{powered});
	return powered;
}
function closeRadioPanel(opts){
	opts=opts||{};
	if(!radioPanelVisible()) return false;
	radioPanel.hidden=true;
	radioTarget=null;
	if(MM.modalInput && MM.modalInput.pop) MM.modalInput.pop('home-radio');
	if(!opts.silent){ try{ if(AUDIO && AUDIO.play) AUDIO.play('uiClose'); }catch(e){} }
	return true;
}
function renderRadioPanel(){
	if(!radioPanel) return;
	const info=currentRadioInfo();
	const live=radioPanel.querySelector('.radioNow');
	if(live){
		live.style.setProperty('--station-accent',info.accent||'#83909e');
		live.dataset.broadcasting=String(!!info.active);
		live.querySelector('.radioNowIcon').textContent=info.icon||'📻';
		live.querySelector('.radioNowName').textContent=info.label||'Radio';
		const details=info.id==='off' ? '' : ((info.genre||'Stacja')+' · '+(info.track||'strojenie sygnału…')+' · ');
		live.querySelector('.radioNowMeta').textContent=details+radioStatusText(info);
	}
	const grid=radioPanel.querySelector('.radioStationGrid'); if(!grid) return;
	grid.innerHTML='';
	for(const station of RADIO_STATIONS){
		const button=document.createElement('button'); button.type='button'; button.className='radioStationCard';
		button.style.setProperty('--station-accent',station.accent);
		button.setAttribute('aria-pressed',String(station.id===info.id));
		button.dataset.active=String(station.id===info.id);
		button.dataset.broadcasting=String(station.id===info.id && info.active);
		const icon=document.createElement('span'); icon.className='radioStationIcon'; icon.textContent=station.icon;
		const copy=document.createElement('span'); copy.className='radioStationCopy';
		const name=document.createElement('strong'); name.textContent=station.label;
		const genre=document.createElement('small'); genre.textContent=station.genre;
		const desc=document.createElement('span'); desc.className='radioStationDesc'; desc.textContent=station.description;
		copy.appendChild(name); copy.appendChild(genre); copy.appendChild(desc);
		if(station.tracks.length){ const tracks=document.createElement('span'); tracks.className='radioStationTracks'; tracks.textContent=station.tracks.join(' · '); copy.appendChild(tracks); }
		const bars=document.createElement('span'); bars.className='radioStationBars'; bars.setAttribute('aria-hidden','true');
		for(let i=0;i<5;i++){ const bar=document.createElement('i'); bar.style.height=(6+((i*7+station.id.length*3)%15))+'px'; bars.appendChild(bar); }
		button.appendChild(icon); button.appendChild(copy); button.appendChild(bars);
		button.addEventListener('click',()=>{
			activateRadioAudio();
			if(!AUDIO || !AUDIO.setRadioStation || !AUDIO.setRadioStation(station.id)) return;
			publishRadioPower();
			try{ if(AUDIO.play) AUDIO.play('uiClick'); }catch(e){}
			msg(station.id==='off' ? '📻 Radio wyłączone' : '📻 '+station.label+' · '+station.genre);
			renderRadioPanel();
		});
...
function radioPanelKeydown(e){
	...
	if(e.key!=='Tab') return;
	const focusable=[...radioPanel.querySelectorAll('button:not([disabled])')];
	if(!focusable.length) return;
	const first=focusable[0], last=focusable[focusable.length-1];
	if(e.shiftKey && document.activeElement===first){ e.preventDefault(); last.focus(); }
	else if(!e.shiftKey && document.activeElement===last){ e.preventDefault(); first.focus(); }
}
function ensureRadioPanel(){
	if(radioPanel) return radioPanel;
	radioPanel=document.createElement('div'); radioPanel.id='radioPanel'; radioPanel.hidden=true;
	radioPanel.setAttribute('role','dialog'); radioPanel.setAttribute('aria-modal','true'); radioPanel.setAttribute('aria-labelledby','radioPanelTitle');
	const dialog=document.createElement('div'); dialog.className='radioDialog';
	const head=document.createElement('div'); head.className='radioHead';
	const titleWrap=document.createElement('div'); titleWrap.className='radioTitleWrap';
	const eyebrow=document.createElement('span'); eyebrow.textContent='WYPOSAŻENIE DOMOWE · ODBIORNIK';
	const title=document.createElement('strong'); title.id='radioPanelTitle'; title.textContent='Wybierz brzmienie domu';
	const subtitle=document.createElement('small'); subtitle.textContent='Każda stacja jest generowana na żywo i ma własny gatunek.';
	titleWrap.appendChild(eyebrow); titleWrap.appendChild(title); titleWrap.appendChild(subtitle);
	const close=document.createElement('button'); close.type='button'; close.className='radioClose'; close.setAttribute('aria-label','Zamknij radio'); close.textContent='×'; close.addEventListener('click',()=>closeRadioPanel());
	head.appendChild(titleWrap); head.appendChild(close); dialog.appendChild(head);
	const live=document.createElement('div'); live.className='radioNow';
	const liveIcon=document.createElement('span'); liveIcon.className='radioNowIcon';
	const liveCopy=document.createElement('span'); liveCopy.className='radioNowCopy';
	const liveName=document.createElement('strong'); liveName.className='radioNowName';
	const liveMeta=document.createElement('span'); liveMeta.className='radioNowMeta';
	liveCopy.appendChild(liveName); liveCopy.appendChild(liveMeta); live.appendChild(liveIcon); live.appendChild(liveCopy); dialog.appendChild(live);
	const grid=document.createElement('div'); grid.className='radioStationGrid'; grid.setAttribute('role','group'); grid.setAttribute('aria-label','Stacje radiowe'); dialog.appendChild(grid);
...
function openRadioPanel(target){
	if(!target || !Number.isFinite(Number(target.x)) || !Number.isFinite(Number(target.y))) return false;
	if(!AUDIO || !AUDIO.setRadioStation){ msg('Radio nie może złapać sygnału'); return false; }
	radioTarget={x:Number(target.x),y:Number(target.y)};
	ensureRadioPanel();
	activateRadioAudio();
	publishRadioPower();
...
function nearestInteractiveRadio(range){
	return FURNISHINGS && FURNISHINGS.findNearestRadio ? FURNISHINGS.findNearestRadio(player,getTile,range||3) : null;
}
...
// inactive slot's remembered item (switching slots never changes it), dimmed
...
// ult charge on the active weapon (bow/spear hold progress while charging,
// bedrock pick durability on slot 1).
const AMMO_LOW=15;
const wepBarSlotCache={};
document.querySelectorAll('#weaponBar .wepSlot').forEach(el=>{
	wepBarSlotCache[el.getAttribute('data-wkey')]={
		el,
		icon:el.querySelector('.wicon'),
...
function updateWeaponBar(){
	...
	Object.keys(wepBarSlotCache).forEach(k=>{
		...
		if(k==='1'){
			...
			const preview=(c && INV.selectedWeaponForCategory)? INV.selectedWeaponForCategory(c.id) : ((cat && c && cat.key===k)? it : list[0]);
			...
			if(!preview){
				...
				if(k==='4' && slot.icon) slot.icon.textContent='🔥';
			}else if(c.id==='melee'){
...
function buildHotbarTip(i){
	...
	frag.appendChild(hudTipNode('tipHint','Klawisz '+hotbarKeyLabel(i)+' · LPM na wolnym polu: przytrzymaj i przeciągnij · PPM stawia pojedynczo'));
	return frag;
}
...
// Soot graffiti (G): paint the selected stencil at the cursor cell; Shift+G
// cycles the stencil. Costs 1 sadza per mark. In hero mode the paint goes out
// as a 'gfx' intent (host validates), the soot is spent from OUR pouch truth
// and the local mark is a prediction the gfx plane confirms.
let graffitiStencil=0;
const GRAFFITI_STENCIL_NAMES={arrow:'strzałka',x:'X',heart:'serce',dot:'kropka'};
function paintGraffitiAtCursor(cycle){
	if(!GRAFFITI) return false;
	if(MM.ghostMode && !MM.ghostHeroIntents) return false; // a watcher has no hands
	const glyphs=GRAFFITI.GLYPHS||['arrow','x','heart','dot'];
	if(cycle){
		graffitiStencil=(graffitiStencil+1)%glyphs.length;
		msg('Szablon sadzy: '+(GRAFFITI_STENCIL_NAMES[glyphs[graffitiStencil]]||glyphs[graffitiStencil]));
		return true;
	}
	if((inv.soot|0)<1){ msg('Brak sadzy — przebiegnij gęsty czarny nalot albo rozkrusz węgiel (recepta).'); return false; }
	let tx,ty;
	if(lastPointer.has){
		const w=screenToWorld(lastPointer.x,lastPointer.y);
		tx=Math.floor(w.x); ty=Math.floor(w.y);
	} else { tx=Math.floor(player.x)+(player.facing<0?-1:1); ty=Math.floor(player.y); }
	if(Math.abs(tx+0.5-player.x)>6 || Math.abs(ty+0.5-player.y)>6){ msg('Za daleko na malowanie.'); return false; }
	const glyph=glyphs[graffitiStencil]||'arrow';
	const dir=lastPointer.has ? (tx+0.5>=player.x?1:-1) : (player.facing<0?-1:1);
	if(!GRAFFITI.canPaintAt(tx,ty,getTile)){ msg('Tu nie ma podłoża na znak.'); return false; }
	if(MM.ghostHeroIntents){
		if(!MM.ghostHeroIntents.gfx || !MM.ghostHeroIntents.gfx(tx,ty,glyph,dir)) return false;
		inv.soot=(inv.soot|0)-1;
		GRAFFITI.paintAt(tx,ty,glyph,dir,getTile);
		updateInventory();
		return true;
	}
	if(!GRAFFITI.paintAt(tx,ty,glyph,dir,getTile)) return false;
	inv.soot=(inv.soot|0)-1;
	updateInventory();
	noteSaveActivity();
	return true;
...
window.addEventListener('keydown',e=>{ if(isEditableTarget(e.target)) return; if(modalInputOpen()){ releaseGameplayInput(); return; } noteSaveActivity(); const k=KEYBINDS.translate(e.key.toLowerCase()); const code=(e.code||'').toLowerCase(); keys[k]=true; if(code) keys[code]=true; if(!e.repeat) queueJumpInput(k); if(k==='s' || k==='arrowdown') trapdoorDropBufferT=TRAPDOOR_DROP_BUFFER; if(k==='escape'){ closeHotSelect(); }
 ...
 if(['5','6','7','8','9','0'].includes(e.key)){
	...
	// Every diagnostic/cheat key, including the harmless F3 counter, is armed by
	// the same explicit developer-toolbox state so gameplay keys stay isolated.
	const debugKeysEnabled=debugShortcutsEnabled();
	...
	if(debugKeysEnabled && k==='k'&&!keysOnce.has('k')){ // force spawn a chest at feet (cycle tiers)
		keysOnce.add('k'); const tiers=['common','uncommon','rare','epic','legendary']; const names=['zwykłą','niezwykłą','rzadką','epicką','legendarną']; const idx=Math.floor(performance.now()/1000)%tiers.length; if(DROPS&&DROPS.spawnChest){ DROPS.spawnChest(player.x,player.y-1,tiers[idx],{source:'debug',vx:0,vy:0}); msg('Debug: zrzucono skrzynię '+names[idx]); } }
	// Debug chest placement (L)
	if(debugKeysEnabled && k==='l'&&!keysOnce.has('l')){ keysOnce.add('l'); const r=Math.random(); const tier=r>0.99?'legendary':r>0.95?'epic':r>0.85?'rare':r>0.65?'uncommon':'common'; if(DROPS&&DROPS.spawnChest){ DROPS.spawnChest(player.x,player.y-0.5,tier,{source:'debug'}); msg('Zrzucono skrzynię ('+tier+')'); } }
	if(debugKeysEnabled && k==='m'&&!keysOnce.has('m')){ toggleMap(); keysOnce.add('m'); }
	...
	if(k==='e'&&!keysOnce.has('e')){
		...
		// SMR inspection outranks loot: answering the reactor's alarm is why you
		// walked up to it (host-only — a guest's world writes go through intents)
		if(!mechWants && !MM.ghostHeroIntents && SMR && SMR.wantsInteractKey && SMR.wantsInteractKey(player) && SMR.inspectNear(player)){
			noteSaveActivity();
		} else if(!mechWants && !MM.ghostHeroIntents && DROPS && DROPS.pickupNearest && DROPS.pickupNearest(player)){
			noteSaveActivity();
		} else if(!MM.ghostHeroIntents && MECHS && MECHS.toggleBoard && MECHS.toggleBoard(player,getTile)){
			noteSaveActivity();
			saveState();
		} else {
			const radio=nearestInteractiveRadio(3);
			if(radio) openRadioPanel(radio);
		}
	}
	...
	if(!debugKeysEnabled && k==='g'&&!keysOnce.has('g')){ paintGraffitiAtCursor(e.shiftKey); keysOnce.add('g'); }
	if(['arrowup','arrowdown','w',' '].includes(k)) e.preventDefault(); });
window.addEventListener('keyup',e=>{ if(modalInputOpen()){ releaseGameplayInput(); return; } noteSaveActivity(); const k=KEYBINDS.translate(e.key.toLowerCase()); const code=(e.code||'').toLowerCase(); keys[k]=false; if(code) keys[code]=false; keysOnce.delete(k); if(!turboKeyHeld()) endTurboEnergyHold(); });
...
let houseComfortHudSignature='';
function updateHouseComfortHud(status){
	const hud=document.getElementById('houseComfortHud');
	if(!hud) return;
	if(!status || !status.ok){
		hud.hidden=true;
	...
	const mult=Math.max(1,Number(status.comfortMult)||1);
	const cap=Math.max(mult,Number(status.comfortCap)||(HOUSE_HEALING.config && HOUSE_HEALING.config.comfortMultCap)||3);
	const progress=Math.max(0,Math.min(1,Number(status.comfortProgress)||((mult-1)/Math.max(.01,cap-1))));
	const count=Math.max(0,Number(status.furnishingCount)||0);
	const types=Math.max(0,Number(status.furnishingTypes)||0);
	const healPct=Math.max(0,Number(status.healRateFrac)||0)*100;
	const breakdown=Array.isArray(status.furnishingBreakdown) ? status.furnishingBreakdown.slice(0,3) : [];
	const signature=[mult.toFixed(3),progress.toFixed(3),count,types,healPct.toFixed(3),breakdown.map(x=>x.tile+':'+x.count).join(',')].join('|');
	hud.hidden=false;
	if(signature===houseComfortHudSignature) return;
	houseComfortHudSignature=signature;
	const value=document.getElementById('houseComfortValue');
	const fill=document.getElementById('houseComfortFill');
	const meter=document.getElementById('houseComfortMeter');
	const meta=document.getElementById('houseComfortMeta');
	const items=document.getElementById('houseComfortItems');
	if(value) value.textContent='×'+mult.toFixed(2);
	if(fill) fill.style.width=Math.round(progress*100)+'%';
	if(meter) meter.setAttribute('aria-valuenow',String(Math.round(progress*100)));
	if(meta) meta.textContent=count+' '+(count===1?'element':'elementów')+' · '+types+' '+(types===1?'typ':'typów')+' · '+healPct.toFixed(2)+'% HP/s · limit ×'+cap.toFixed(0);
	if(items){
		items.textContent=breakdown.length ? breakdown.map(entry=>{
			const info=INFO[entry.tile]||{};
			const name=RES_LABEL[info.drop]||info.drop||(entry.chair?'Fotel':'Wyposażenie');
			return name+(entry.count>1?' ×'+entry.count:'');
		}).join(' · ') : 'Pusty, ale bezpieczny dom';
	}
	hud.setAttribute('aria-label','Komfort domu '+mult.toFixed(2)+' z limitem '+cap.toFixed(0));
}
function resetHouseHealingRuntimeState(){
	const fresh=HOUSE_HEALING && HOUSE_HEALING.createState ? HOUSE_HEALING.createState() : {scanT:0, inside:false, last:null, reportAcc:0, wasHealing:false};
	for(const key of Object.keys(houseHealingState)) delete houseHealingState[key];
	Object.assign(houseHealingState,fresh);
	houseHealMsgAt=0;
...
function resetWorldTransitionRuntime(){
	...
	player.vx=0; player.vy=0; player.onGround=false; player.jumpCount=0;
	releaseGameplayInput();
}
...
function updateHouseHealing(dt){
	...
	updateHouseComfortHud(res && res.status);
	...
	if(res && res.started){
		...
		if(now-houseHealMsgAt>14000){
			houseHealMsgAt=now;
			const furnishings=(res.status && res.status.furnishingCount)|0;
			const mult=Math.max(1,Number(res.status && res.status.comfortMult)||1);
			msg(furnishings>0 ? 'Domowe zacisze: '+furnishings+' elementów wyposażenia · regeneracja ×'+mult.toFixed(2) : 'Dom: dach, ściany i światło powoli leczą rany');
		}
	}
}
...
function isLadderTileId(t){ return t===T.LADDER || t===T.BEDROCK_LADDER; }
function hasLadderAt(x,y){
	return hasInfrastructureTile(x,y,T.LADDER) || hasInfrastructureTile(x,y,T.BEDROCK_LADDER) || isLadderTileId(getTile(x,y));
}
...
function solarHeroSkyOpen(){
	...
	for(const tx of probes){
		for(let yy=top; yy>=WORLD_MIN_Y; yy--){
			...
			if(t!==T.AIR && t!==T.WATER && t!==T.WIRE && !isPowerCableTileId(t) && t!==T.WATER_PIPE) return false;
		}
	}
	return true;
}
...
function physics(dt){
	...
	if(SURVIVAL && SURVIVAL.updateSwimChill){
		// a geothermal spring is the one warm bath in the game — soaking in
		// warm water never chills (geothermal.updateHero also heals slowly)
		const warmBath = !!(GEOTHERMAL && GEOTHERMAL.heroInWarmWater && GEOTHERMAL.heroInWarmWater());
		const swimming = inWater && !player.onGround && !ridingFloatingBoat && !godMode && !warmBath;
...
function updateDropPreview(){
	...
	if(info.kind==='gear'){
		...
		dropPreviewEl.appendChild(dropPreviewNode('dpTitle','📦 '+(info.label||'Skrzynia')));
		dropPreviewEl.appendChild(dropPreviewNode('dpSub','Ciężki łup · otwórz na miejscu'));
	} else {
		dropPreviewEl.appendChild(dropPreviewNode('dpTitle',(info.glyph||'📦')+' '+(info.label||info.res)+(info.qty>1?' ×'+info.qty:'')));
	}
	const hint=info.kind==='chest'
		? (info.inReach?'LPM lub E otwiera · broń także rozbija zamek':'Podejdź bliżej, aby otworzyć')
		: (info.inReach?'LPM podnosi · E zbiera wszystko w zasięgu':'Podejdź bliżej, aby podnieść');
	dropPreviewEl.appendChild(dropPreviewNode('dpHint',hint));
...
const CRAFT_RESULT_KEY_RECIPES=new Map();
const CRAFT_SEEN_TILE_RECIPES=(()=>{ // numeric tile id -> [recipe,...]
	const map=new Map();
	RECIPES.forEach(r=>{
		if(!r.out) return;
		if(!CRAFT_RESULT_KEY_RECIPES.has(r.out)) CRAFT_RESULT_KEY_RECIPES.set(r.out,[]);
		CRAFT_RESULT_KEY_RECIPES.get(r.out).push(r);
...
function noteCraftResultSeen(resourceKey,opts){
	...
	for(const r of recipes){
		const fresh=CRAFT_MODEL.noteSeenResult(r.id);
	...
	if(!(opts&&opts.announce===false)){
		...
		msg('🔎 '+prefix+unlocked.slice(0,3).map(r=>r.name||r.id).join(', ')+(unlocked.length>3?' (+'+(unlocked.length-3)+')':''));
		try{ if(MM.audio && MM.audio.play) MM.audio.play('craft'); }catch(e){}
	}
	noteCategoryDiscoveries({});
	renderCraftPanel();
...
MM.noteCraftResultSeen=noteCraftResultSeen;
...
function scanCraftablesInView(dt){
	...
	for(let ty=sy0; ty<=sy1; ty++){
		for(let tx=sx0; tx<=sx1; tx++){
			...
			for(const r of recipes){
				const fresh=noteCraftResultSeen(r.out,{announce:false,persist:false});
				fresh.forEach(nr=>unlocked.push(nr.name||nr.id));
			}
		}
	}
	if(unlocked.length){
...
let mineBossTarget=null; // opaque {boss,part} anchor resolved against the boss's live position
function stopMining(){ mining=false; mineBoatTarget=null; mineBossTarget=null; mineBtn.classList.remove('on'); }
...
function bossMineTargetAt(tx,ty){
	if(!BOSSES || !BOSSES.partAt || getInfrastructureTile(tx,ty)!==T.AIR) return null;
	return BOSSES.partAt(tx,ty);
}
function resolveBossMinePoint(){
	if(!mineBossTarget || !BOSSES || !BOSSES.resolvePartTarget) return null;
	return BOSSES.resolvePartTarget(mineBossTarget);
}
function breakBossPart(){
	const p=resolveBossMinePoint();
	if(!p || !BOSSES.mineTarget){ stopMining(); return false; }
	mineTx=p.tx; mineTy=p.ty;
	if(!canPhysicallyTargetTile(mineTx,mineTy)) return false;
	const hit=BOSSES.mineTarget(mineBossTarget);
	if(!hit) return false;
	spawnBurst(p.x*TILE,p.y*TILE,0,{color:(p.part&&p.part.color)||'#888'});
	noteSaveActivity();
	return true;
}
function updateBossMining(dt){
	const p=resolveBossMinePoint();
	if(!p){ stopMining(); resumeHeldMining(); return; }
	mineTx=p.tx; mineTy=p.ty;
	if(!withinReach(mineTx,mineTy,MINE_REACH)){ stopMining(); return; }
	if(!canPhysicallyTargetTile(mineTx,mineTy)){ stopMining(); resumeHeldMining(); return; }
	if(godMode){ if(breakBossPart()) stopMining(); return; }
	const info=mineInfoForId(p.tile);
	if(!info || !canMineTileWithCurrentTool(p.tile,mineTx,mineTy)){ stopMining(); return; }
	try{ if(MM.audio && MM.audio.play) MM.audio.play('dig',{x:p.x,y:p.y}); }catch(e){}
	updateMeteorPickMiningFx(dt,p.tile);
	const mineMult=(MM.activeModifiers && MM.activeModifiers.mineSpeedMult)||1;
	mineTimer += dt * tools[player.tool] * mineMult;
	const need=p.tile===T.BEDROCK ? BEDROCK_MINE_NEED : Math.max(0.1,info.hp/6);
	if(mineTimer>=need && breakBossPart()){
		const soundPoint={x:p.x,y:p.y};
		stopMining();
		try{ if(MM.audio && MM.audio.play) MM.audio.play('break',soundPoint); }catch(e){}
		resumeHeldMining();
	}
}
...
if(CHESTS && CHESTS.setDroppedOpenHandler) CHESTS.setDroppedOpenHandler((drop,res)=>{
	if(!drop || !res) return;
	try{ if(MM.audio && MM.audio.play) MM.audio.play('chest',{x:drop.x,y:drop.y}); }catch(e){}
	lastChestOpen={t:performance.now(),x:Math.floor(drop.x),y:Math.floor(drop.y)};
	if(!MM.onLootGained && window.updateDynamicCustomization) window.updateDynamicCustomization();
	const tierPl=({common:'zwykła',uncommon:'niezwykła',rare:'rzadka',epic:'epicka',legendary:'legendarna'})[res.tier]||res.tier;
	if(res.spawned>0) msg('Skrzynia '+tierPl+' wyrzuciła '+res.spawned+' przedm. — podnieś je (E / klik)!');
	else msg('Skrzynia '+tierPl+': +'+res.items.length+' przedm. (I aby zobaczyć)');
	spawnBurst(drop.x*TILE,drop.y*TILE,res.tier);
	noteSaveActivity();
	saveState();
});
...
function mineTileIdAt(tx,ty){
	...
	const bossPart=BOSSES && BOSSES.partAt ? BOSSES.partAt(tx,ty) : null;
	if(bossPart) return bossPart.tile;
...
function startMineAt(tx,ty,opts){
	...
	const bossTarget=bossMineTargetAt(tx,ty);
	const t=mineTileIdAt(tx,ty);
...
function dismantleDynamoAt(tx,ty){
	...
	// A collapsed dynamo settles as separate casing/slot entities. Only a fragment
	// containing the unique rotor slot may yield the original device; otherwise
	// each orphan casing would mint another complete dynamo when dismantled.
	const refund=(DYNAMO && DYNAMO.dismantleRefundForCells)
		? DYNAMO.dismantleRefundForCells(undoCells)
		: (undoCells.some(cell=>cell.oldId===T.DYNAMO_SLOT) ? 1 : 0);
	const removed=[];
	for(const cell of cells){
		const oldId=getTile(cell.x,cell.y);
		if(!setForegroundConfirmed(cell.x,cell.y,T.AIR)){
			for(let i=removed.length-1;i>=0;i--){
				const row=removed[i];
				if(setForegroundConfirmed(row.cell.x,row.cell.y,row.oldId)) notifyStructureTileChanged(row.cell.x,row.cell.y,T.AIR,row.oldId);
			}
			return false;
		}
		removed.push({cell,oldId});
	...
	if(!godMode && refund) inv.dynamo=(inv.dynamo||0)+refund;
	pushUndo(tx,ty,T.DYNAMO,T.AIR,'breakDynamo',refund?[{key:'dynamo',n:refund}]:[],{cells:undoCells,refund});
	updateInventory();
	return true;
}
...
function breakTileByCompanion(tx,ty,expectedTile){
	...
	if(overId!==T.AIR){
		...
		const removed=clearInfrastructureConfirmed(tx,ty,overId);
	...
	if(miningTargetsConstructionBackground(tx,ty)){
		...
		const removed=clearConstructionBackgroundConfirmed(tx,ty);
		if(!removed) return false;
		const drops=awardTileDrops(info);
...
function breakMinedTile(){
	...
	if(overId!==T.AIR){
		const info=INFO[overId];
		if(!info) return false;
		const removed=clearInfrastructureConfirmed(mineTx,mineTy,overId);
		if(!removed) return false;
		const drops=awardTileDrops(info);
	...
	const bossPart=BOSSES && BOSSES.partAt ? BOSSES.partAt(mineTx,mineTy) : null;
	if(bossPart && BOSSES.mineAt){
		const hit=BOSSES.mineAt(mineTx,mineTy);
		if(hit){
			spawnBurst((mineTx+0.5)*TILE,(mineTy+0.5)*TILE,0,{color:(bossPart.part&&bossPart.part.color)||'#888'});
			noteSaveActivity();
			return true;
		}
	}
	if(miningTargetsConstructionBackground(mineTx,mineTy)){
		...
		const info=INFO[bgId];
		if(!info) return false;
		const removed=clearConstructionBackgroundConfirmed(mineTx,mineTy);
		if(!removed) return false;
		const drops=awardTileDrops(info);
	...
	if(AVALANCHE && AVALANCHE.disturb && (tId===T.SNOW || tId===T.TOXIC_SNOW || getTile(mineTx,mineTy-1)===T.SNOW)) AVALANCHE.disturb(mineTx,mineTy,1.2,getTile);
	...
	maybeChainVeinBreak(tId);
	return true;
}
// vein pickaxe head: after a real break, one MATCHING neighbour sometimes
// cracks along — routed through breakMinedTile itself (depth-1 recursion), so
...
function maybeChainVeinBreak(tId){
	...
	const info=INFO[tId];
	if(info && (info.chestTier || info.cache || info.unmineable)) return;
	for(const [dx,dy] of [[1,0],[-1,0],[0,1],[0,-1]]){
		const nx=mineTx+dx, ny=mineTy+dy;
		if(mineTileIdAt(nx,ny)!==tId) continue;
		if(!canMineTileWithCurrentTool(tId,nx,ny) || !canPhysicallyTargetTile(nx,ny)) continue;
		const ox=mineTx, oy=mineTy;
		mineTx=nx; mineTy=ny; veinChainDepth=1;
		try{
			if(breakMinedTile()) spawnBurst((nx+0.5)*TILE,(ny+0.5)*TILE,0,{color:'#c9d6ff'});
		} finally { veinChainDepth=0; mineTx=ox; mineTy=oy; }
		break;
	}
}
function instantBreak(){
	...
	if(t===T.AIR || !canMineTileWithCurrentTool(t,mineTx,mineTy)){ stopMining(); return; }
	if(breakMinedTile()) stopMining();
}
...
function updateMining(dt){
	...
	if(mineBossTarget){ updateBossMining(dt); return; }
	...
	// lucky pickaxe head: rolled ONCE per targeted block — on success the first
	// bite finishes it (bedrock keeps its own durability contract, no shortcuts)
	const luckKey=mineTx+','+mineTy;
	if(mineLuckyKey!==luckKey){
		mineLuckyKey=luckKey;
		mineLuckyArmed=curId!==T.BEDROCK && equippedPickPerk()==='lucky' && Math.random()<PICK_PERKS.lucky.chance;
	}
	if(mineLuckyArmed && mineTimer<need){
		mineLuckyArmed=false;
		mineTimer=need;
		spawnBurst((mineTx+0.5)*TILE,(mineTy+0.5)*TILE,0,{color:'#ffd96a'});
	}
...
function beginBuildStroke(e,tx,ty,opts){
	...
	buildStroke={
		...
		visited:new Set([Math.floor(tx)+','+Math.floor(ty)])
	};
	e.preventDefault();
	return true;
}
function continueBuildStroke(e){
	...
	const target=screenToWorldTile(e.clientX,e.clientY);
	if(!Number.isFinite(target.tx) || !Number.isFinite(target.ty)) return true;
	const cells=rasterizeTileLine(stroke.lastTx,stroke.lastTy,target.tx,target.ty,BUILD_STROKE_CELL_LIMIT);
	if(!cells.length) return true;
	for(let i=1;i<cells.length;i++){
		const cell=cells[i], k=cell.tx+','+cell.ty;
		if(stroke.visited.has(k)) continue;
		stroke.visited.add(k);
		if(stroke.visited.size>4096){ stroke.visited.clear(); stroke.visited.add(k); }
		if(!godMode && !haveBlocksFor(selectedTileId())){
			clearBuildStroke(e.pointerId);
			break;
		}
		const placement=canPlaceAt(cell.tx,cell.ty);
		if(!placement.ok) continue;
		tryPlace(cell.tx,cell.ty);
	}
	if(buildStroke===stroke){
		const tail=cells[cells.length-1];
		stroke.lastTx=tail.tx;
		stroke.lastTy=tail.ty;
	}
	return true;
}
...
function tryRotateWaterPumpAt(tx,ty){
	if(getTile(tx,ty)!==T.WATER_PUMP) return false;
	...
	const previous=PUMPS.orientationAt(tx,ty,getTile);
	const next=PUMPS.rotateDir(previous);
	if(!PUMPS.setOrientationAt(tx,ty,next,getTile)){
		msg('Nie udało się obrócić pompy');
		return true;
	}
	// Reuse the chosen direction for the next placement and repaint the cached
	// tile immediately; changing machine metadata does not bump the world tile.
	pumpOrientation=next;
	try{ localStorage.setItem(PUMP_ORIENTATION_KEY,pumpOrientation); }catch(e){}
	markChunkRenderDirty(Math.floor(tx/CHUNK_W),ty,1);
	noteSaveActivity();
	saveState();
	msg('Pompa obrócona: '+pumpOrientationLabel(next));
...
function tryToggleFluidPipeModeAt(tx,ty){
	...
	if(!godMode && !withinReach(tx,ty,PLACE_REACH)){ msg('Za daleko'); return true; }
	const blocked=infrastructureTargetBlockedReason(tx,ty,T.WATER_PIPE);
	if(blocked){ msg(blocked); return true; }
	if(!PUMPS || !PUMPS.togglePipeModeAt){ msg('Rura nie odpowiada'); return true; }
	const next=PUMPS.togglePipeModeAt(tx,ty,getFluidNetworkTile);
	if(!next){ msg('Nie udało się zmienić funkcji rury'); return true; }
	noteSaveActivity();
	saveState();
	msg(next==='intake' ? 'Rura: WLOT WODY — turkusowa kratka zasysa wodę' : 'Rura: zwykły odcinek — końcówka nadal działa automatycznie');
	try{ if(MM.audio && MM.audio.play) MM.audio.play('place',{x:tx+0.5,y:ty+0.5}); }catch(e){}
	return true;
}
...
function tryEatWorldFoodAt(tx,ty){
	...
	const blocked=blockedTargetReason(tx,ty);
	if(blocked){ msg(blocked); return true; }
	const probeInv={}; probeInv[effect.key]=1;
	const preview=FOOD.applyFoodEffect({hp:player.hp,maxHp:player.maxHp},probeInv,tileId,{godMode:false,immunityMode});
	if(!preview.ok) return finishFoodUse(effect,preview);
	// Consuming world food is not a reversible tile edit: restoring the food
	// without rolling back its health/status effect would permit infinite reuse.
	if(!setForegroundConfirmed(tx,ty,T.AIR)) return false;
...
function tryToggleForegroundToBackgroundAt(tx,ty,id){
	...
	const placed=setConstructionBackgroundConfirmed(tx,ty,id);
	if(!placed){ msg('Nie mozna przelaczyc'); return true; }
	if(!setForegroundConfirmed(tx,ty,T.AIR)){
		clearConstructionBackgroundConfirmed(tx,ty);
		msg('Nie mozna przelaczyc'); return true;
	}
...
function canToggleBackgroundToForegroundAt(tx,ty,id){
	...
	// Water displaced by the foreground block still exists elsewhere in the
	// fluid simulation. Undo must reveal AIR and let that unit flow back instead
	// of materialising a second unit at the original cell.
	return {ok:true,
		oldForeground:(isGasTileId(cur)||cur===T.WATER)?T.AIR:cur,
		previousForeground:isGasTileId(cur)?T.AIR:cur,
		replacedWater:cur===T.WATER, pressureCells};
}
function tryToggleBackgroundToForegroundAt(tx,ty,id){
	...
	const cleared=clearConstructionBackgroundConfirmed(tx,ty);
	if(!cleared){ msg('Nie mozna przelaczyc'); return true; }
	if(v.replacedWater && getTile(tx,ty)===T.WATER){
		let displaced=false;
		try{ displaced=!!(WATER && WATER.displaceAt && WATER.displaceAt(tx,ty,getTile,setTile)); }catch(e){ displaced=false; }
		if(!displaced){ setConstructionBackgroundConfirmed(tx,ty,id); msg('Brak miejsca na wypchniecie wody'); return true; }
	}
	if(!setForegroundConfirmed(tx,ty,id)){ setConstructionBackgroundConfirmed(tx,ty,id); msg('Nie mozna przelaczyc'); return true; }
	pushUndo(tx,ty,v.oldForeground,id,'toggleBackgroundToForeground',null,{tileId:id});
	notifyForegroundTilePlacedForLayerToggle(tx,ty,v.previousForeground,id,v.replacedWater);
	wakeConstructionBackgroundChanged(tx,ty);
	saveState();
...
function isStableConstructionSupportAt(x,y){
	const foreground=getTile(x,y), background=getConstructionBackgroundTile(x,y);
	return isBuildAnchorTile(foreground) || isStableMachineSupport(foreground)
		|| isBuildAnchorTile(background) || isStableMachineSupport(background);
}
...
function canPlaceLadderAt(tx,ty,cur,id){
	id=isLadderTileId(id)?id:T.LADDER;
	if(hasLadderAt(tx,ty)) return {ok:false, id, overlay:true, reason:'Juz jest taka instalacja'};
	if(cur===T.LAVA) return {ok:false, id, overlay:true, reason:'Lawa blokuje drabinke'};
	...
	const rule=canPlaceLadderFixture({
		...
		naturalSolidBlocked,
		oneEndSupport:id===T.BEDROCK_LADDER,
		maxRun:id===T.BEDROCK_LADDER ? Math.max(128,worldMaxY()-worldMinY()+2) : 128,
		hasLadder:hasLadderAt,
		hasBacking:ladderTargetHasBuiltBackingAt,
		hasAnchor:ladderAnchorAt
	});
	if(!rule.ok) return {ok:false, id, overlay:true, reason:rule.reason||'Brak podparcia'};
...
function canPlaceDynamoAt(tx,ty){
	if(!worldCellInBounds(tx,ty)) return {ok:false,id:T.DYNAMO,structure:'dynamo',cells:[],reason:'Poza granica swiata'};
	...
	if(!Array.isArray(cells) || cells.length!==3) return {ok:false,id:T.DYNAMO,structure:'dynamo',cells:[],reason:'Nieprawidlowy plan dynama'};
	const uniqueCells=new Set();
	for(const cell of cells){
		if(!cell || !Number.isInteger(cell.x) || !Number.isInteger(cell.y) || !worldCellInBounds(cell.x,cell.y)) return {ok:false,id:T.DYNAMO,structure:'dynamo',cells,reason:'Dynamo nie miesci sie w swiecie'};
		const k=Math.floor(cell.x)+','+Math.floor(cell.y);
		if(uniqueCells.has(k)) return {ok:false,id:T.DYNAMO,structure:'dynamo',cells,reason:'Nieprawidlowy plan dynama'};
		uniqueCells.add(k);
	}
...
function canPlaceInfrastructureAt(tx,ty,id,remoteContext){
	...
	const cur=getTile(tx,ty);
	if(isLadderTileId(id) && hasLadderAt(tx,ty)) return {ok:false, id, overlay:true, reason:'Juz jest taka instalacja'};
	if(hasInfrastructureTile(tx,ty,id)) return {ok:false, id, overlay:true, reason:'Juz jest taka instalacja'};
	if(isPowerCableTileId(id) && getInfrastructureTiles(tx,ty).some(t=>isPowerCableTileId(t))) return {ok:false,id,overlay:true,reason:'W tym miejscu jest juz przewod elektryczny'};
...
function placeDynamoStructure(v){
	...
	const undoCells=cells.map(cell=>{
		...
		const oldId=(isGasTileId(oldRaw)||oldRaw===T.WATER)?T.AIR:oldRaw;
		return {x:cell.x,y:cell.y,oldId,newId:cell.t};
	});
	// The rotor slot may be placed into flowing water. Move that unit before any
	// casing mutation so failure cannot leave a half-built dynamo or lose fluid.
	for(const cell of cells){
		if(getTile(cell.x,cell.y)!==T.WATER) continue;
		let displaced=false;
		try{ displaced=!!(WATER && WATER.displaceAt && WATER.displaceAt(cell.x,cell.y,getTile,setTile)); }catch(e){ displaced=false; }
		if(!displaced){ msg('Brak miejsca na wypchniecie wody'); return false; }
	}
	const placed=[];
	try{
		for(let i=0;i<cells.length;i++){
			...
			if(getTile(cell.x,cell.y)!==cell.t) throw new Error('dynamo write rejected');
			placed.push({cell,undo:undoCells[i]});
			notifyStructureTileChanged(cell.x,cell.y,undoCells[i].oldId,cell.t);
		}
	}catch(e){
		for(let i=placed.length-1;i>=0;i--){
			const row=placed[i];
			try{ setTile(row.cell.x,row.cell.y,row.undo.oldId); notifyStructureTileChanged(row.cell.x,row.cell.y,row.cell.t,row.undo.oldId); }catch(err){}
		}
		msg('Nie udalo sie zbudowac dynama');
		return false;
	}
	consumeFor(T.DYNAMO);
...
function canPlaceAt(tx,ty){
	if(!worldCellInBounds(tx,ty)) return {ok:false,reason:'Poza granica swiata'};
	const cur=getTile(tx,ty);
	...
	let furnishingPlacement=null;
	if(!chest && isFurnishingTileId(id) && FURNISHINGS && FURNISHINGS.validatePlacement){
		furnishingPlacement=FURNISHINGS.validatePlacement(id,tx,ty,getTile,{getBackground:getConstructionBackgroundTile});
		if(furnishingPlacement && furnishingPlacement.applies && !furnishingPlacement.ok){
...
function tryPlace(tx,ty){
	...
	const displacePlacedWater=()=>{
		if(id===T.WATER || getTile(tx,ty)!==T.WATER) return true;
		try{ return !!(WATER && WATER.displaceAt && WATER.displaceAt(tx,ty,getTile,setTile)); }catch(e){ return false; }
	};
	if(v.chest){
		if(!displacePlacedWater()){ msg('Brak miejsca na wypchniecie wody'); return false; }
		const tier=(INFO[id]&&INFO[id].chestTier)||'common';
		const d=DROPS&&DROPS.spawnChest ? DROPS.spawnChest(tx+0.5,ty+0.35,tier,{source:'god_mode',vx:0,vy:0.2}) : null;
		if(d){ saveState(); return true; }
		return false;
	}
	if(v.boat){
	...
	if(v.overlay){
		...
		const placed=setInfrastructureConfirmed(tx,ty,id);
		if(!placed){ msg('Nie udalo sie polozyc instalacji'); return false; }
	...
	if(v.background){
		...
		const placed=setConstructionBackgroundConfirmed(tx,ty,id);
		if(!placed){ msg('Nie udalo sie polozyc tla'); return false; }
	...
	if(!displacePlacedWater()){ msg('Brak miejsca na wypchniecie wody'); return false; }
	if(!setForegroundConfirmed(tx,ty,id)){ msg('Nie udalo sie postawic przedmiotu'); return false; }
	let machineRegistered=true;
	if(id===T.VENDING_MACHINE && VENDING && VENDING.onPlaced) machineRegistered=VENDING.onPlaced(tx,ty,getTile)!==false;
	if(id===T.SPRING_PLATFORM && SPRING_PLATFORMS && SPRING_PLATFORMS.onTileChanged) machineRegistered=SPRING_PLATFORMS.onTileChanged(tx,ty,prev,id,getTile)!==false && machineRegistered;
	if(!machineRegistered){
		// A full persistent-machine registry must not consume an item and leave an
		// inert tile. Roll back when possible; if the world write itself refuses the
		// rollback, account for the tile below so inventory still follows reality.
		if(setForegroundConfirmed(tx,ty,undoPrev)){
			if(id===T.VENDING_MACHINE && VENDING && VENDING.onTileRemoved) VENDING.onTileRemoved(tx,ty);
			if(id===T.SPRING_PLATFORM && SPRING_PLATFORMS && SPRING_PLATFORMS.onTileChanged) SPRING_PLATFORMS.onTileChanged(tx,ty,id,undoPrev,getTile);
			msg('Limit aktywnych maszyn zostal osiagniety');
			return false;
		}
	}
...
function keepFailedUndo(e){
	if(e) undoStack.push(e);
	msg('Nie mozna cofnac');
}
function undoDropsFor(e){
	if(Array.isArray(e && e.drops)) return e.drops;
	const info=e && INFO[e.oldId];
	return info && info.drop ? [{key:info.drop,n:1}] : [];
}
function undoDropsAvailable(drops){
	if(godMode) return true;
	const need=new Map();
	for(const d of drops||[]){
		if(!d || !d.key) continue;
		const n=Math.max(1,(d.n|0)||1);
		need.set(d.key,(need.get(d.key)||0)+n);
	}
	for(const [key,n] of need){ if((inv[key]||0)<n) return false; }
	return true;
}
function reclaimUndoDrops(drops){
	if(godMode) return;
	for(const d of drops||[]){
		if(!d || !d.key) continue;
		inv[d.key]=Math.max(0,(inv[d.key]||0)-Math.max(1,(d.n|0)||1));
	}
}
function restoreUndoStructureCells(cells){
	const changed=[];
	for(const cell of cells){
		if(!setForegroundConfirmed(cell.x,cell.y,cell.oldId)){
			for(let i=changed.length-1;i>=0;i--){
				const prev=changed[i];
				setForegroundConfirmed(prev.x,prev.y,prev.newId);
			}
			return false;
		}
		changed.push(cell);
	}
	for(const cell of cells) notifyStructureTileChanged(cell.x,cell.y,cell.newId,cell.oldId);
	return true;
}
function undoLastChange(){
	...
	if(e.kind==='placeDynamo' && Array.isArray(e.cells)){
		if(e.cells.length!==3 || e.cells.some(cell=>!cell || getTile(cell.x,cell.y)!==cell.newId)){ keepFailedUndo(e); return; }
		if(!restoreUndoStructureCells(e.cells)){ keepFailedUndo(e); return; }
	...
	if(e.kind==='breakDynamo' && Array.isArray(e.cells)){
		const refund=Math.max(0,Math.min(1,Number(e.refund===undefined?1:e.refund)||0));
		if(!e.cells.length || e.cells.length>5 || e.cells.some(cell=>!cell || getTile(cell.x,cell.y)!==cell.newId) || (!godMode && (inv.dynamo||0)<refund)){ keepFailedUndo(e); return; }
		if(!restoreUndoStructureCells(e.cells)){ keepFailedUndo(e); return; }
		if(!godMode && refund) inv.dynamo=Math.max(0,(inv.dynamo||0)-refund);
		updateInventory(); updateHotbarCounts(); saveState(); msg('CofniÄ™to'); return;
	}
	if(e.kind==='placeOverlay'){
		if(!hasInfrastructureTile(e.x,e.y,e.newId)){ keepFailedUndo(e); return; }
		const removed=clearInfrastructureConfirmed(e.x,e.y,e.newId);
		if(!removed){ keepFailedUndo(e); return; }
	...
	if(e.kind==='breakOverlay'){
		...
		if(hasInfrastructureTile(e.x,e.y,e.oldId) || !undoDropsAvailable(drops)){ keepFailedUndo(e); return; }
		const placed=setInfrastructureConfirmed(e.x,e.y,e.oldId);
		if(!placed){ keepFailedUndo(e); return; }
		reclaimUndoDrops(drops);
		updateInventory(); updateHotbarCounts(); saveState(); msg('Cofnieto'); return;
	}
	if(e.kind==='toggleForegroundToBackground'){
		const tile=e.tileId||e.oldId;
		if(getPlayerConstructionBackgroundTile(e.x,e.y)!==tile){ keepFailedUndo(e); return; }
		const cur=getTile(e.x,e.y);
		if(!isReplaceableNaturalOpenTile(cur,false)){ keepFailedUndo(e); return; }
		const cleared=clearConstructionBackgroundConfirmed(e.x,e.y);
		if(!cleared){ keepFailedUndo(e); return; }
		if(cur===T.WATER){
			let displaced=false;
			try{ displaced=!!(WATER && WATER.displaceAt && WATER.displaceAt(e.x,e.y,getTile,setTile)); }catch(err){ displaced=false; }
			if(!displaced){
				setConstructionBackgroundConfirmed(e.x,e.y,tile);
				keepFailedUndo(e); return;
			}
		}
		const prev=isGasTileId(cur)?T.AIR:cur;
		if(!setForegroundConfirmed(e.x,e.y,tile)){
			setConstructionBackgroundConfirmed(e.x,e.y,tile);
			keepFailedUndo(e); return;
		}
	...
	if(e.kind==='toggleBackgroundToForeground'){
		...
		if(getTile(e.x,e.y)!==tile){ keepFailedUndo(e); return; }
		const placed=setConstructionBackgroundConfirmed(e.x,e.y,tile);
		if(!placed){ keepFailedUndo(e); return; }
		if(!setForegroundConfirmed(e.x,e.y,e.oldId)){
			clearConstructionBackgroundConfirmed(e.x,e.y);
			keepFailedUndo(e); return;
		}
	...
	if(e.kind==='placeBackground'){
		if(getConstructionBackgroundTile(e.x,e.y)!==e.newId){ keepFailedUndo(e); return; }
		const removed=clearConstructionBackgroundConfirmed(e.x,e.y);
		if(!removed){ keepFailedUndo(e); return; }
		const k=invKeyForTile(e.newId);
		if(k && !godMode) inv[k]=(inv[k]||0)+1;
		wakeConstructionBackgroundChanged(e.x,e.y);
	...
	if(e.kind==='breakBackground'){
		const drops=undoDropsFor(e);
		if(getConstructionBackgroundTile(e.x,e.y)!==T.AIR || !undoDropsAvailable(drops)){ keepFailedUndo(e); return; }
		const placed=setConstructionBackgroundConfirmed(e.x,e.y,e.oldId);
		if(!placed){ keepFailedUndo(e); return; }
		reclaimUndoDrops(drops);
		wakeConstructionBackgroundChanged(e.x,e.y);
		updateInventory(); updateHotbarCounts(); saveState(); msg('Cofnieto'); return;
	...
	const cur=getTile(e.x,e.y);
	if(cur!==e.newId){ keepFailedUndo(e); return; }
	if(e.kind==='place'){
		...
		const k=invKeyForTile(e.newId);
		if(k && !godMode) inv[k] = (inv[k]||0)+1;
	} else if(e.kind==='break' || e.kind==='companionBreak'){
		const drops=undoDropsFor(e);
...
const HOT_SELECT_GROUPS=[
	...
	{id:'utility',label:'Instalacje',tiles:['WIRE','COPPER_WIRE','SILVER_WIRE','WATER_PIPE','LADDER','BEDROCK_LADDER','WATER_PUMP','TRANSISTOR','TORCH','WEATHERVANE','LIGHTNING_ROD','RESPAWN_TOTEM']},
...
canvas.addEventListener('pointerdown',e=>{
	...
	if(e.button===0){
		...
		if(!e.shiftKey && dxRange<=3 && dyRange<=3 && getTile(tx,ty)===T.RADIO && openRadioPanel({x:tx,y:ty})) return;
		...
		// A valid free build cell starts the RMB placement stroke (drag paints a
		// line). Occupied/invalid cells keep the classic single secondary action.
		if(beginBuildStroke(e,tx,ty)) return;
...
function draw(){ // Background first
 ...
 drawInfrastructureOverlays(sx,sy,viewX,viewY,{only:[T.LADDER,T.BEDROCK_LADDER]});
 drawDoorOpenOverlays(sx,sy,viewX,viewY);
 // Screens, water, holograms, orbiting particles and other home details stay
 // alive even though the furniture body itself is baked into chunk caches.
 if(FURNISHINGS && FURNISHINGS.drawEffects) FURNISHINGS.drawEffects(ctx,TILE,sx,sy,viewX,viewY,getTile,worldFxVisible,HOME_FURNISHING_POWER);
 // weather layer: clouds, rain/snow, lightning, evaporation mist (world-space, sky)
 // aurora curtains: upper-sky bands behind the clouds (cold clear nights)
 ...
 // drift gales: autumn leaf storms / snow blizzards — haze + streamed flakes.
 // The spawn gate mirrors the loop's sim hold: while pause/ceremony freezes
 // updateParticles, streaming more flakes would pile a frozen wall to the cap.
 if(SOFT_DRIFTS && SOFT_DRIFTS.drawStorm) SOFT_DRIFTS.drawStorm(ctx,TILE,sx,sy,viewX,viewY,!paused && !uiOverlayHold());
 // morning fog: milk-white dawn haze in the valleys, burns off with the sun
 if(SKY_MOODS && SKY_MOODS.drawFog) SKY_MOODS.drawFog(ctx,TILE,sx,sy,viewX,viewY,camRenderX,camRenderY);
 ...
 // SMR status glyphs: running / inspection alarm / SCRAMmed (over the tile)
 if(SMR && SMR.draw) SMR.draw(ctx,TILE,worldFxVisible);
 // weather instruments: live wind arrow on vanes + charge/flash on rods
 if(WEATHER_INSTRUMENTS && WEATHER_INSTRUMENTS.draw) WEATHER_INSTRUMENTS.draw(ctx,TILE,worldFxVisible);
 ...
 drawInfrastructureOverlays(sx,sy,viewX,viewY,{exclude:[T.LADDER,T.BEDROCK_LADDER]});
 ...
 drawFogOverlay(sx,sy,viewX,viewY,{camX:camRenderX,camY:camRenderY,shake:screenShake});
	...
	if(!ceremonyHold){ drawTreasureCompassHUD(); drawMinimap(); }
...
function minimapTileColor(t){
	...
	if(isFurnishingTileId(t)) return (INFO[t] && INFO[t].color) || '#a9743c';
	if(t===T.GLASS) return '#9deeff';
	if(t===T.WIRE) return '#c56f32';
	if(t===T.COPPER_WIRE) return '#d68535';
	if(t===T.SILVER_WIRE) return '#d9ecff';
	if(t===T.SILVER_ORE) return '#aeb8c8';
	if(t===T.SILVER_INGOT) return '#e7eef5';
...
// The compass card stays OFF-SCREEN while the pendant is merely tuning or has
// nothing — it appears (fading in, at the LEFT EDGE where nothing else lives)
// only when a treasure is actually sensed, or briefly after the radar button
// asks for a readout. No signal = no UI noise (owner ruling).
let compassShowK=0, compassShowAt=0;
function drawTreasureCompassHUD(){
	const level=treasureCompassLevel();
	const target=level>0 ? TREASURE_SCANNER.target() : null;
	const now=performance.now();
	const wanted=level>0 && (!!target || now<radarFlash);
	const dtK=Math.min(0.1,Math.max(0,(now-compassShowAt)/1000)); compassShowAt=now;
	compassShowK+=( (wanted?1:0) - compassShowK )*Math.min(1,dtK*6);
	if(compassShowK<0.02){ compassShowK=Math.max(0,compassShowK); return false; }
	const metrics=TREASURE_SCANNER.metrics();
	const compact=W<=760;
	const narrow=W<=480;
	const width=Math.min(compact?236:268,Math.max(narrow?188:216,Math.round(W*0.34)));
	const height=narrow?40:(compact?48:56);
	// Left edge, above the vertical midline: the top band belongs to status
	// rows and the craft tracker, the bottom to vitals — the flank is free.
	const slide=(1-compassShowK)*24;
	const left=Math.round(8-slide);
	const top=Math.round(Math.max(8,Math.min(H-height-8,H*0.30)));
	const radius=narrow?13:(compact?15:17);
	const cx=left+(narrow?24:30),cy=top+height*0.5;
	ctx.save();
	ctx.globalAlpha=Math.min(1,compassShowK);
	ctx.fillStyle='rgba(6,13,23,0.84)';
	ctx.strokeStyle=target?(TREASURE_COMPASS.CATEGORY_COLORS[target.category]||'#8de8ff'):'rgba(150,173,199,0.55)';
	ctx.lineWidth=1;
	ctx.beginPath();
	if(ctx.roundRect) ctx.roundRect(left+0.5,top+0.5,width-1,height-1,13);
	else ctx.rect(left+0.5,top+0.5,width-1,height-1);
	ctx.fill(); ctx.stroke();
	TREASURE_COMPASS.draw(ctx,target,{x:cx,y:cy,radius,time:now*0.001,alpha:1});
	ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.font='700 7px system-ui'; ctx.fillStyle='rgba(226,241,255,0.82)';
	const tick=radius*0.67;
	ctx.fillText('N',cx,cy-tick); ctx.fillText('S',cx,cy+tick); ctx.fillText('W',cx-tick,cy); ctx.fillText('E',cx+tick,cy);
	ctx.textAlign='left';
	const textLeft=left+(narrow?44:56);
	ctx.font=narrow?'700 12px system-ui':'700 13px system-ui'; ctx.fillStyle='#f3f8ff';
	const main=target ? target.label : (metrics.progress<1?'Strojenie…':'Brak sygnału');
	const maxChars=narrow?16:20;
	ctx.fillText(main.length>maxChars?main.slice(0,maxChars-1)+'…':main,textLeft,top+(narrow?14:(compact?18:20)));
	ctx.font=narrow?'10px system-ui':'11px system-ui'; ctx.fillStyle=target?'#b9ccda':'#8395a8';
	const sub=target
		? Math.max(1,Math.round(target.distance))+' bloków · '+treasureDirectionLabel(target)
		: 'zasięg '+metrics.range;
	ctx.fillText(sub,textLeft,top+(narrow?29:(compact?34:38)));
	ctx.restore();
	return true;
}
...
(function(){
	...
	function tradeRow(entry,kind){
		...
		if(entry.furnishingKey){
			row.classList.add('furnishingOffer','furnishingTier'+Math.max(1,Math.min(4,entry.furnishingTier|0)));
			row.dataset.furnishingKey=entry.furnishingKey;
		}
		const name=document.createElement('span'); name.className='tName';
		name.textContent=entry.icon+' '+entry.label+(entry.furnishingTier?' · T'+entry.furnishingTier:'');
		name.title=entry.description||entry.label;
	...
	function renderPanel(){
		...
		stock.offers.forEach(o=>{
			if(o.furnishingKey) noteCraftResultSeen(o.furnishingKey,{source:'trader'});
			buySec.appendChild(tradeRow(o,'buy'));
		});
...
document.getElementById('visionBtn')?.addEventListener('click',toggleSpecialVision);
refreshHeroLampButton();
refreshVisionButton();
...
// Developer override only. Player-facing auto-loot is an equipment modifier
// (lootMagnetLevel), so this control deliberately stays out of the pause menu.
(function(){
	const toggle=document.getElementById('debugAutoPickupCheckbox');
	if(!toggle || !DROPS) return;
	const read=()=>DROPS.debugAutoPickup ? DROPS.debugAutoPickup() : (DROPS.autoPickup ? DROPS.autoPickup() : false);
	const write=on=>DROPS.setDebugAutoPickup ? DROPS.setDebugAutoPickup(on) : (DROPS.setAutoPickup && DROPS.setAutoPickup(on));
	toggle.checked=!!read();
	toggle.addEventListener('change',()=>{ write(toggle.checked); msg('Debug auto-zbierania: '+(toggle.checked?'ON':'OFF')); });
})();
...
if(MM.ui && MM.ui.injectSmrDebugPanel) MM.ui.injectSmrDebugPanel({
	kit:()=>{ inv.graphite=(inv.graphite|0)+8; inv.graphene=(inv.graphene|0)+4; inv.smrCell=(inv.smrCell|0)+1; inv.radioactiveOre=(inv.radioactiveOre|0)+4; inv.transistor=(inv.transistor|0)+6; inv.meteoricIron=(inv.meteoricIron|0)+4; inv.silverWire=(inv.silverWire|0)+8; updateInventory(); checkCraftingAvailability({silent:true}); return true; },
	place:()=>{
		if(!SMR) return false;
		const px=Math.floor(player.x)+ (player.facing<0?-2:2), py=Math.floor(player.y);
		for(const [dx,dy] of [[0,0],[0,-1],[1,0],[-1,0],[0,1]]){
			const x=px+dx, y=py+dy;
			if(getTile(x,y)!==T.AIR) continue;
			setTile(x,y,T.SMR_CELL);
			if(getTile(x,y)===T.SMR_CELL){ notifyStructureTileChanged(x,y); noteSaveActivity(); saveState(); return true; }
		}
		return false;
	},
	alarm:()=>{ if(!SMR || !SMR._debug || !SMR._debug.cellNear) return false; const c=SMR._debug.cellNear(player.x,player.y,20); return c ? SMR._debug.forceAlarm(c.x,c.y) : false; },
	inspect:()=>{ if(!SMR || !SMR.inspectNear) return false; const c=SMR._debug && SMR._debug.cellNear ? SMR._debug.cellNear(player.x,player.y,20) : null; return c ? SMR.inspectAt(c.x,c.y) : false; },
	metrics:()=> (SMR && SMR.metrics) ? SMR.metrics() : null
}, menuPanel);
if(MM.ui && MM.ui.injectDriftDebugPanel) MM.ui.injectDriftDebugPanel({
	gale:(mat)=>{ if(!SOFT_DRIFTS || !SOFT_DRIFTS.startStorm) return null; return SOFT_DRIFTS.startStorm(mat, 60, 0.95, {source:'debug'}); },
	stopGale:()=>{ if(!SOFT_DRIFTS || !SOFT_DRIFTS.stopStorm) return false; return SOFT_DRIFTS.stopStorm(); },
	seed:(mat)=>{ if(!SOFT_DRIFTS || !SOFT_DRIFTS.seedAround) return 0; return SOFT_DRIFTS.seedAround(Math.floor(player.x), 9, mat, 6, getTile, setTile); },
	clear:()=>{ if(!SOFT_DRIFTS || !SOFT_DRIFTS.clearAll) return 0; return SOFT_DRIFTS.clearAll(); },
	metrics:()=> (SOFT_DRIFTS && SOFT_DRIFTS.metrics) ? SOFT_DRIFTS.metrics() : null
}, menuPanel);
if(MM.ui && MM.ui.injectNatureDebugPanel) MM.ui.injectNatureDebugPanel({
	// a deep snow shelf on a step next to the hero — instant avalanche fodder
	slope:()=>{
		const dir=player.facing<0?-1:1;
		const bx=Math.floor(player.x)+dir*4, by=Math.floor(player.y);
		let placed=0;
		for(let i=0;i<8;i++){
			const x=bx+dir*i;
			const base=by-Math.floor(i/2);
			for(let d=0;d<4;d++){
				const y=base-d;
				if(getTile(x,y)!==T.AIR) continue;
				setTile(x,y,T.SNOW);
				if(getTile(x,y)===T.SNOW){ placed++; if(FALLING && FALLING.afterPlacement) FALLING.afterPlacement(x,y); }
			}
		}
		if(placed){ noteSaveActivity(); }
		return placed;
	},
	avalanche:()=>{ if(!AVALANCHE || !AVALANCHE.disturb) return 0; return AVALANCHE.disturb(Math.floor(player.x)+(player.facing<0?-5:5), player.y, 2, getTile); },
	icicles:()=>{ if(!ICICLES || !ICICLES.seedAround) return 0; return ICICLES.seedAround(player.x, player.y, getTile); },
	dropIcicles:()=>{ if(!ICICLES || !ICICLES.dropAll) return 0; return ICICLES.dropAll(); },
	freeze:()=>{ if(!THIN_ICE_SIM || !THIN_ICE_SIM.freezeAround) return 0; const n=THIN_ICE_SIM.freezeAround(player.x, 40, getTile, setTile); if(n) noteSaveActivity(); return n; },
	spring:()=>{ if(!GEOTHERMAL || !GEOTHERMAL.buildSpring) return false; const ok=GEOTHERMAL.buildSpring(player.x, player.y, getTile, setTile); if(ok) noteSaveActivity(); return ok; },
	fog:()=>{ if(!SKY_MOODS || !SKY_MOODS.startMood) return false; SKY_MOODS.startMood('fog'); msg('Mgla nadciaga — i sama sie rozejdzie'); return true; },
	aurora:()=>{ if(!SKY_MOODS || !SKY_MOODS.startMood) return false; SKY_MOODS.startMood('aurora'); msg('Zorza rozblyska — i sama przygasnie'); return true; },
	instruments:()=>{ inv.weathervane=(inv.weathervane|0)+2; inv.lightningRod=(inv.lightningRod|0)+2; inv.soot=(inv.soot|0)+10; inv.steel=(inv.steel|0)+6; inv.silverWire=(inv.silverWire|0)+4; updateInventory(); checkCraftingAvailability({silent:true}); return true; },
	strikeRod:()=>{
		if(!WEATHER_INSTRUMENTS || !CLOUDS || !CLOUDS.strike) return false;
		const m=WEATHER_INSTRUMENTS._debug && WEATHER_INSTRUMENTS._debug.rods;
		let best=null, bd=Infinity;
		if(m) for(const c of m.values()){ const d=Math.abs(c.x-player.x); if(d<bd){ bd=d; best=c; } }
		if(!best || bd>40) return false;
		return !!CLOUDS.strike(best.x+0.5,getTile,setTile);
	},
	nature:()=>{ if(!MOBS || !MOBS._debugNature) return false; MOBS._debugNature.forcePass(); return true; },
	paint:()=>{
		if(!GRAFFITI || !GRAFFITI.paintAt) return false;
		inv.soot=(inv.soot|0)+1;
		const glyphs=GRAFFITI.GLYPHS||['arrow'];
		const g=glyphs[Math.floor(Math.random()*glyphs.length)];
		for(const [dx,dy] of [[2,0],[-2,0],[2,-1],[-2,-1],[0,1]]){
			const x=Math.floor(player.x)+dx, y=Math.floor(player.y)+dy;
			if(GRAFFITI.paintAt(x,y,g,player.facing<0?-1:1,getTile)){ inv.soot--; updateInventory(); noteSaveActivity(); return true; }
		}
		return false;
	},
	metrics:()=>{
		const av=(AVALANCHE && AVALANCHE.metrics)?AVALANCHE.metrics():{triggered:0};
		const ic=(ICICLES && ICICLES.metrics)?ICICLES.metrics():{hanging:0};
		const ti=(THIN_ICE_SIM && THIN_ICE_SIM.metrics)?THIN_ICE_SIM.metrics():{panes:0};
		const ge=(GEOTHERMAL && GEOTHERMAL.metrics)?GEOTHERMAL.metrics():{pools:0};
		const sm=(SKY_MOODS && SKY_MOODS.metrics)?SKY_MOODS.metrics():{fog:0,aurora:0};
		const wi=(WEATHER_INSTRUMENTS && WEATHER_INSTRUMENTS.metrics)?WEATHER_INSTRUMENTS.metrics():{rods:0,energy:0};
		const gr=(GRAFFITI && GRAFFITI.metrics)?GRAFFITI.metrics():{marks:0};
		const na=(MOBS && MOBS.natureMetrics)?MOBS.natureMetrics():{drinks:0,lurks:0,flees:0,baths:0};
		return {avalanche:av.triggered, icicles:ic.hanging, panes:ti.panes, pools:ge.pools,
			fog:sm.fog, aurora:sm.aurora, rods:wi.rods, rodEnergy:wi.energy, marks:gr.marks, nature:na};
	}
}, menuPanel);
...
// Spawn
function placePlayerAtRespawnSpot(spot){
	if(!spot || !Number.isFinite(Number(spot.x)) || !Number.isFinite(Number(spot.y)) || !worldYInBounds(Number(spot.y))) return false;
	const x=Number(spot.x), y=Number(spot.y);
	ensureChunkAtY(Math.floor(x/CHUNK_W),y);
	player.x=x; player.y=y; player.vx=0; player.vy=0; player.onGround=false;
	return true;
}
function placePlayer(skipMsg,opts){
	...
	if(dest && dest.spot){
		...
		if(!placePlayerAtRespawnSpot(spot)) return;
...
function debugHazardReplaceable(t){
	...
	if(t===T.GOLD_ORE || t===T.SILVER_ORE || t===T.SILVER_INGOT || t===T.DIAMOND || t===T.IRIDIUM || t===T.UFO_CONCRETE || t===T.METEORIC_IRON || t===T.RADIOACTIVE_ORE || t===T.ALIEN_BIOMASS || t===T.METEOR_DUST || t===T.ANTIMATTER_CRYSTAL) return false;
...
MM.ghostBridge={
	...
	// truth (graffiti.canPaintAt) exactly like the solo paint path does.
	ghostHeroGraffiti:(tx,ty,glyph,dir)=>{
		if(!GRAFFITI || !GRAFFITI.paintAt) return false;
		if(!worldCellInBounds(tx,ty)) return false;
		const ok=GRAFFITI.paintAt(tx,ty,glyph,dir,getTile);
...
function runGameStep(dt,ts){
	...
	updateOceanHint(dt);
	let homePowerChanged=false;
	if(FURNISHINGS && FURNISHINGS.updatePower) homePowerChanged=!!FURNISHINGS.updatePower(dt,player,getTile,HOME_FURNISHING_POWER);
	if(FURNISHINGS && FURNISHINGS.updateAudio) FURNISHINGS.updateAudio(dt,player,getTile,AUDIO,HOME_FURNISHING_POWER);
	if(homePowerChanged && radioPanelVisible()) renderRadioPanel();
	...
	if(SOFT_DRIFTS && SOFT_DRIFTS.update) SOFT_DRIFTS.update(dt, player, getTile, setTile);
...
function runHeroStep(dt,ts){
	...
	if(FISHING && FISHING.update) FISHING.update(dt, player, getTile);
	// hero comfort systems (no world writes): a warm spring computed from the
	// REPLICA tiles heals the guest hero exactly like solo; sky moods (fog /
	// aurora + its energy trickle) are deterministic from the shared clock
	if(GEOTHERMAL && GEOTHERMAL.updateHero) GEOTHERMAL.updateHero(dt, player, getTile);
	if(SKY_MOODS && SKY_MOODS.update) SKY_MOODS.update(dt, player);
	updateMining(dt);
...
if(!window.__lootNoticeInit){
	...
	function currentUpgradeDeltaText(cmp){
		if(!cmp || !cmp.item) return 'Przedmiot nie jest już dostępny';
		if(cmp.equipped && cmp.equipped.id===cmp.item.id) return '✓ Obecnie założony';
		if(cmp.equippedComparable && cmp.equippedDelta!=null){
			if(cmp.equippedDelta>0) return '▲ +'+cmp.equippedDelta+' Moc vs noszone: '+lootNoticeName(cmp.equipped);
			if(cmp.equippedDelta<0) return '▼ '+cmp.equippedDelta+' Moc vs noszone: '+lootNoticeName(cmp.equipped);
			return '= Ta sama Moc co noszone: '+lootNoticeName(cmp.equipped);
		}
		if(!cmp.equipped) return 'Pierwszy przedmiot do tego slotu';
		return 'Inna kategoria niż noszone: '+lootNoticeName(cmp.equipped);
	}
	function refreshUpgradeNoticeCard(card){
		...
		const equipBtn=card.querySelector('.upEquip');
		if(kicker) kicker.textContent=worthy ? '⬆ Znaleziono lepszy przedmiot' : (equipped ? '✓ Założony przedmiot' : 'Znaleziony przedmiot');
		if(delta) delta.textContent=currentUpgradeDeltaText(cmp);
		if(equipBtn){
			equipBtn.disabled=!item || equipped;
			equipBtn.textContent=!item ? 'Niedostępny' : (equipped ? 'Założony' : 'Załóż');
		}
		card.classList.toggle('noLongerUpgrade',!worthy);
		card.setAttribute('aria-label',(worthy?'Lepszy przedmiot: ':equipped?'Założony przedmiot: ':'Znaleziony przedmiot: ')+(item ? (item.name||item.id) : card.dataset.itemId));
		return !!item;
	}
	...
	function showUpgradeNotice(item,cmp){
		...
		card.appendChild(upgradeNode('upDelta',currentUpgradeDeltaText(cmp)));
		if(INV && INV.statChips){
...
window.regenWorldSameSeed = function(){ try{ resetWorldTransitionRuntime(); if(MOBS && MOBS.clearAll) try{ MOBS.clearAll(); }catch(e){} if(COMPANIONS && COMPANIONS.reset) try{ COMPANIONS.reset(); }catch(e){} if(WORLD && WORLD.clear) WORLD.clear(); if(typeof chunkCanvases!=='undefined') chunkCanvases.clear(); if(typeof chunkRenderDirty!=='undefined') chunkRenderDirty.clear(); if(WORLD && WORLD.clearHeights) WORLD.clearHeights(); if(FALLING && FALLING.reset) FALLING.reset(); if(MECHS && MECHS.reset) MECHS.reset(); if(TREES && TREES.reset) TREES.reset(); if(WATER && WATER.reset) WATER.reset(); if(GASES && GASES.reset) GASES.reset(); if(WIND && WIND.reset) WIND.reset(); if(SEASONS && SEASONS.reset) SEASONS.reset(); if(DYNAMO && DYNAMO.reset) DYNAMO.reset(); if(SOLAR && SOLAR.reset) SOLAR.reset(); if(TELEPORTERS && TELEPORTERS.reset) TELEPORTERS.reset(); if(PUMPS && PUMPS.reset) PUMPS.reset(); if(TURRETS && TURRETS.reset) TURRETS.reset(); if(SPRING_PLATFORMS && SPRING_PLATFORMS.reset) SPRING_PLATFORMS.reset(); if(VENDING && VENDING.reset) VENDING.reset(); if(CLOUDS && CLOUDS.reset) CLOUDS.reset(); if(BOSSES && BOSSES.reset) BOSSES.reset(); if(GUARDIANS && GUARDIANS.reset) GUARDIANS.reset(); if(UNDERGROUND && UNDERGROUND.reset) UNDERGROUND.reset(); if(SKY_GUARDIAN && SKY_GUARDIAN.reset) SKY_GUARDIAN.reset(); if(AFTERMATH && AFTERMATH.reset) AFTERMATH.reset(); if(NPCS && NPCS.reset) NPCS.reset(); if(GENERATED_NPCS && GENERATED_NPCS.reset) GENERATED_NPCS.reset(); if(GRASS && GRASS.reset) GRASS.reset(); if(PARTICLES && PARTICLES.reset) PARTICLES.reset(); if(FIRE && FIRE.reset) FIRE.reset(); if(WEAPONS && WEAPONS.reset) WEAPONS.reset(); if(MEAT && MEAT.reset) MEAT.reset(); if(DROPS && DROPS.reset) DROPS.reset(); if(VOLCANO && VOLCANO.reset) VOLCANO.reset(); if(ATOMIC_WINTER && ATOMIC_WINTER.reset) ATOMIC_WINTER.reset(); if(TERRAIN_TRAPS && TERRAIN_TRAPS.reset) TERRAIN_TRAPS.reset(); if(UFO && UFO.reset) UFO.reset(); if(TASKS && TASKS.reset) TASKS.reset(); if(INVASIONS && INVASIONS.reset) INVASIONS.reset(); if(METEORITES && METEORITES.reset) METEORITES.reset(); if(PLANTS && PLANTS.reset) PLANTS.reset();
	...
	try{ if(FOG && FOG.importSeen) FOG.importSeen([]); if(FOG && FOG.setRevealAll) FOG.setRevealAll(false); if(MM.ui && MM.ui.updateMapButton && FOG && FOG.getRevealAll) MM.ui.updateMapButton(FOG.getRevealAll()); }catch(e){}
	RESOURCE_KEYS.forEach(k=>{ inv[k]=0; }); inv.tools.stone=inv.tools.meteor=inv.tools.diamond=inv.tools.bedrock=false; inv.bedrockPickDurability=0; player.tool='basic'; hotbarIndex=0; player.xp=0; player.energy=0; player.soot=0; player._smokeTint=0; if(HERO_LAMP && HERO_LAMP.reset) HERO_LAMP.reset(); if(VISION_MODES && VISION_MODES.reset) VISION_MODES.reset(); TREASURE_SCANNER.reset(); TREASURE_DROP_WINDOW.reset(); heroLampButtonKey=''; visionButtonKey=''; treasureButtonKey=''; refreshHeroLampButton(); refreshVisionButton(); refreshTreasureButton(); if(PROGRESS && PROGRESS.reset) PROGRESS.reset(); applyProgressHp(); applyHeroEnergyCapacity(); clearRespawnTotems(); clearHealingShelters(); grave=null; saveGrave();
